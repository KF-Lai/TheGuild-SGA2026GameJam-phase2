...
Use this skill when the user invokes $brainstorm or asks for work matching the description. This skill is compatible with both Codex and GPT contexts.
...
- Treat legacy slash-command examples as invocation examples; prefer $brainstorm <args> when referring to this skill.
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `direct user question`** at key decision points throughout brainstorming:
   ...
   `direct user question` to capture the decision with concise labels.
