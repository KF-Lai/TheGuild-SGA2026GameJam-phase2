...
Use this skill when the user invokes $design-ds or asks for work matching the description. This skill is compatible with both Codex and GPT contexts.
...
- Treat legacy slash-command examples as invocation examples; prefer $design-ds <args> when referring to this skill.
...
全自動 skill：不互動、不問問題、不分階段確認；流程跑到底僅輸出產出清單與索引更新摘要。**兩個例外**：(1) 模型能力不足時先降級為規劃或審查模式；(2) data-index 列表與 GDD 內容不符時中止並回報。
