...
# Ensures critical state survives summarization
...
if [ -f "$STATE_FILE" ]; then
    ...
    echo "Consider maintaining production/session-state/active.md for recovery."
fi
...
for f in design/gdd/*.md; do
    [ -f "$f" ] || continue
    INCOMPLETE=$(grep -n -E "TODO|WIP|PLACEHOLDER|\[TBD\]" "$f" 2>/dev/null)
