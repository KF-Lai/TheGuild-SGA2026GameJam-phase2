...
description: "AI Pair 兩輪優化主入口（CC 審查 + Codex 實作 + Unity MCP 測試閘）。輸入支援單檔／多檔／資料夾／-r 遞迴／GDD ID。單組直接執行 Phase 1-5；多組自動轉呼 auto-code-optimize-batch。當前 branch 直接 commit，不自動 push。"
...
2. **列印確認區塊**
   ...
   工具預期會碰到：
     - Codex MCP 寫入：<target scripts parent dirs>
   ...
   無人值守建議：
     ...
     [B] 暫存目錄已預授權；Codex MCP 不觸發 prompt（預期不被打斷）
...
2. **發送 spec 給 Codex**（首次 dispatch）
   - `mcp__codex__codex`：`cd=repo_root`、`sandbox="read-only"`、`SESSION_ID=null`、`return_all_messages=true`
