...
FT-08 GachaSystem 與 FT-12 StaffSystem 共用的系統常數 key-value 表，集中管理保底閾值、trash 機率、auto refresh 間隔、effect 聚合上限、狀態切換冷卻等可調參數，避免 SystemConstants 因子系統爆量。
...
- **註冊位置**：FT-08 / FT-12 各自於 `RegisterTables()` 中分組注册各自 owner key（FT-08 §6.4 / FT-12 §6.4）
...
- **消費者**：
  - FT-08 GachaSystem：`PITY_THRESHOLD`（§3.4 / §4.1.8）/ `TRASH_ROLL_RATE_AT_RARITY_1`（§3.5.3）/ `MIN_AUTO_REFRESH_INTERVAL_SEC`（§4.1.3）/ `INTERVIEW_AUTO_REFRESH_INTERVAL_L*`（§4.1.3 / §6.5）/ `INTERVIEW_SLOT_COUNT_L*`（§7.2）
  - FT-12 StaffSystem：`EFFECT_MAX_*`（§4.1.3）/ `BUILDING_SWITCH_COOLDOWN_SECONDS`（§3.6.4）/ `REALLOCATING_AUTO_LEAVE_SECONDS`（§3.7）/ `ROSTER_CAP`（Post-Jam §3.3.4）
...
- **CSV 結構**：整檔僅一組 `key` 列 + 一組 `value` 列（`ParseSystemConstants` 限制）；FT-08 / FT-12 共用 owner 透過 `description` 列或 `#` 註解列分組標註，**不可**用「另起一組 `key,...` 列」分隔，否則第二組以後全部讀不到
...
```csv
# === StaffTuning ===
# FT-08 / FT-12 共用 key-value 常數表
# 整檔僅一組 key 列 + 一組 value 列；owner 分組以 # 註解列說明（parser 忽略註解）
# FT-08 owner: PITY_THRESHOLD / TRASH_ROLL_RATE_AT_RARITY_1 / MIN_AUTO_REFRESH_INTERVAL_SEC
# FT-12 owner: BUILDING_SWITCH_COOLDOWN_SECONDS / REALLOCATING_AUTO_LEAVE_SECONDS

key,PITY_THRESHOLD,TRASH_ROLL_RATE_AT_RARITY_1,MIN_AUTO_REFRESH_INTERVAL_SEC,BUILDING_SWITCH_COOLDOWN_SECONDS,REALLOCATING_AUTO_LEAVE_SECONDS
value,60,0.30,3600,21600,43200
description,FT-08 §4.1.8 保底閾值,FT-08 §3.5.3 1★ trash 機率,FT-08 §4.1.3 自動刷新最低間隔秒,FT-12 §3.6.4 進入 Working 冷卻秒,FT-12 §3.7 自動轉休假閾值秒
```
...
#### FT-08 owner（FT-08 §7.2 / §4.1.3 / §6.5）
