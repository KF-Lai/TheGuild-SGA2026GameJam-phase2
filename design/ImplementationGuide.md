...
| 20.B2 | P-02-FSD-B2 Main UI Panels（部分完成）| 部分完成 | 2026-05-03 | 2026-05-03 | **SettingsPanel 已落地**（unity-ui-specialist subagent fallback，Sonnet+high；codex/gpt-mcp token 上限禁用）；SettingsPanel.cs 395 行 + UIText.csv +12 keys + DesktopWindowService.GetUserScaleRange() +15 行 + TheGuild.UI.Panels.asmdef 加 ref TheGuild.UI.Platform.Win32 + DoD_B14 test 反轉（IsNotCreated → IsCreated）；7 個元件（縮放 slider / 還原 button / 螢幕 dropdown / 音量 slider Post-Jam 唯讀 / 語言 dropdown 鎖 zhTW / 最小化 button / 離開遊戲 Destructive confirm）全實作；EC-17 已斷開螢幕前綴 / EC-18 slider clamp 雙重保險；EditMode 290/299 + 9 既有 [Ignore] 全綠 / 0 fail；DoD-B15 待 Standalone Win build manual test。**仍待**：(1) AdventurerRoster D-01 facade 整合（D-01 service 為 stub，C-02 AdventurerFactory.cs:101-103 註釋，待 D-01 service 落地）(2) StoryDialoguePanel DialogueTable 整合（DialogueTable owner 待定）。**意外發現**：CommissionBoard D-02 facade 整合 B1 已落地（CommissionBoardPanel.cs:96 已呼 MissionDatabaseService.Instance.GetMissionText），D-02 systems-index 同步修正為 ✅ |
