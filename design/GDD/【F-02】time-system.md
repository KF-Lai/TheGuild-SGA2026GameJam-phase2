...
>
> **FSD 回註（2026-04-26，源自 `【F-02-FSD】time-system.md` §8.3 方案 A）**：實作層 `OnOfflineResolved` payload **僅含 `offlineSeconds`**，不含 `completedCount`。原因：F-02 不持有任務列表，無法在發布事件時填入正確值。`completedCount` 改由 FT-02 訂閱本事件後另發 `OnOfflineMissionsResolved(offlineSeconds, completedCount, completedMissionIDs[])`（屬 FT-02 規則範圍，見 FT-02 §6.1 FSD 回註）。本表 `OnOfflineResolved` 攜帶資料以 FSD 為準；§4.3 公式中的 `completedCount` 改由 FT-02 提供。
