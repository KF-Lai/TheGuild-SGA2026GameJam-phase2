...
| `raceID` | `int` | FK → RaceTable；`0` = 依職業隨機（C-04 透過 `C-03.GetProfession.raceIDs` 加權抽取，欄位已合併入 `ProfessionTable`） |
...
| Tick 更新 | `TickWoundedRecovery() : void` | C-02 訂閱 F-02 `OnSecondTick`，每秒自行呼叫；將到期的 Wounded → Idle（同時設 `idleSinceTimestamp = NowUTC`） |
...
CreateRandomInstance(rank, professionID, raceID, traitIDs):
    ...
    instance.traitIDs = traitIDs
    instance.factionID = 0                     // neutral
    instance.status = Idle                     // 預設 Idle；但尚未進入名冊，狀態機不在此觸發
    instance.idleSinceTimestamp = 0            // 尚未加入名冊，不設 timestamp；等 AddAdventurer 呼叫後 C-02 才設定
...
| 系統 | 依賴內容 | 使用介面 |
|------|---------|---------|
| FT-01 Recruitment | 加入新冒險者、建立模板實例、隨機生成實例、檢查名冊容量 | `AddAdventurer`、`CreateFromTemplate`、`CreateRandomInstance`（隨機生成）、`IsRosterFull` |
...
| 欄位 | 值 |
|---|---|
| `OwnerKey` | `"c02Adventurers"` |
| `IsCritical` | `true`（名冊缺失導致 FT-02 / FT-03 等後續 owner 無法驗證 instanceID，整檔回退） |

**`Serialize()` 序列化欄位**：

序列化整個 `roster: List<AdventurerInstance>`（每筆含下列欄位）與 `_nextInstanceID`：
...
| `raceID` | `int` | 種族 ID |
| `traitIDs` | `List<int>` | 特質 ID 列表 |
| `factionID` | `int` | 陣營 ID（FT-09 用；0 = 無陣營） |
| `status` | `string` | 狀態（`Idle` / `Dispatched` / `Wounded` / `Dead`） |
| `currentMissionID` | `int` | 派遣中的任務 ID；非 Dispatched 時為 0 |
| `woundedUntilTimestamp` | `long` | 受傷恢復期結束時間戳；非 Wounded 時為 0 |
| `idleSinceTimestamp` | `long` | 最近一次轉為 Idle 的時間戳（FT-03 自主接單依據） |
| `lastAutoPickupTimestamp` | `long` | 最近一次自主接單時間戳（FT-03 節流依據） |
| `_nextInstanceID` | `int` | 下一個可用 instanceID |

**`RestoreFromSave(string ownerJson)` 行為**：

1. 反序列化 `roster` 列表與 `_nextInstanceID`。
2. 逐筆驗證：`templateID > 0` 時呼叫 `F-01.GetTemplate(templateID)` 確認合法；`professionID` 透過 `F-01.Get<ProfessionData>(professionID)` 驗證；`raceID` / `traitIDs` 同理。
3. 驗證失敗的單筆資料：`LogWarning` 並跳過（不加入名冊），不中斷整體還原。
4. 僅當所有筆資料驗證失敗（名冊完全空白且原始資料非空）時，拋例外觸發整檔回退。

**`InitializeAsNewGame()` 預設值**：空名冊（`roster = new List<>()`），`_nextInstanceID = 1`。
