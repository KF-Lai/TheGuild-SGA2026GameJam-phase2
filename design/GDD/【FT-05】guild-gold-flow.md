...
5. **加成查詢策略**（Q5）：**即時查詢**——不訂閱 FT-06 / FT-07 / FT-12 / C-05 的變動事件維護快取，而是每次結算時即時呼叫 `FT12.GetAccountantBonus()` / `FT07.IsAccountantSlotActive()` 等查詢 API。介面保留但 Game Jam 階段可以 null-check（系統未實作時回傳無加成）
...
> **Jam 範疇備忘（Scope Note）— 2026-04-23 更新**
> FT-05 的**委託金流**（預收、結算）於 Jam 版完整實作；**固定支出**（設施維護費 `OnGuildMaintenanceDue`、職員薪水 `OnStaffSalaryDue`）為 **Phase 2** 規格——FT-05 保留完整契約、管線與降級分支（Case 5.4.1），但發布者 FT-07 / FT-12 在 Jam 版**不發布**對應事件，兩條管線因此為 no-op。相關 AC 於 Jam 階段**不驗證實際扣款行為**，僅驗證「無事件輸入 → 無金流、無事件輸出」。

FT-05 Guild Gold Flow 是公會所有金流操作的統一執行者，涵蓋兩大類別：**委託金流**（預收、結算）與**固定支出**（設施維護費、職員薪水）。

**委託金流**：訂閱 `OnCommissionAccepted` 事件執行預收（`AddGold(+baseReward)`），訂閱 FT-04 `OnMissionResolved` 事件執行結算——成功淨得 `baseReward × effectiveCommissionRate + conditionGoldBonus`，失敗淨損 `baseReward × effectivePenaltyRate` 再加 `conditionGoldBonus`。**固定支出**：訂閱 FT-07 `OnGuildMaintenanceDue` 執行設施維護費扣款，訂閱 FT-12 `OnStaffSalaryDue` 執行職員薪水扣款。

所有外流金流（結算賠償、維護費、薪水）透過 F-03 新 API `AddGoldAllowBankruptcy` 執行，允許突破破產門檻觸發 `Warning` / `Bankrupt`，符合「有重量的決策」設計支柱。所有金流事件 payload 皆含 `bankruptcyStateBefore` / `bankruptcyStateAfter` 狀態快照，供 P-03 Notification 辨識「破產警告開始」、「破產觸發」、「負債已結清」三類狀態轉移並選擇對應通知模板。職員加成（FT-12 會計 `+2% 傭金率`、`-2% 賠償率`）與建築加成（FT-07 預備金保險櫃預留擴充）於結算時即時查詢；Game Jam 階段若下游系統尚未實作，FT-05 以預設值降級運作。
...
4. 訂閱 FT-12 `OnStaffSalaryDue`，執行職員薪水扣款並發布 `OnSalaryCharged`
5. 即時查詢 FT-12 / FT-07 的職員加成、建築加成，計算 `effectiveCommissionRate` / `effectivePenaltyRate`
...
- 維護費 / 薪水**金額計算**與**觸發時機**（FT-07 / FT-12 各自決定，透過 `Due` 事件傳入金額與時間戳）
...
| 欄位 | 型別 | 說明 |
|------|------|------|
| `items` | `Dictionary<int, int>` | 各職員薪水細項：`instanceID → salary`（由 FT-12 傳入；key 為 FT-12 內部 `instanceID`，FT-05 僅 iterate sum 不關心 key 語意） |
...
QueryStaffBonuses(breakdown):
    // 會計職員傭金加成（被動，招募即生效）
    breakdown.accountantCommissionBonus = (FT12 == null) ? 0f
                                          : FT12.GetAccountantCommissionBonus()

    // 會計職員賠償加成（需指派至「預備金保險櫃」slot）
    // FT-12 內部判斷：會計已招募 AND 已指派 AND 預備金保險櫃已解鎖
    breakdown.accountantPenaltyBonus    = (FT12 == null) ? 0f
                                          : FT12.GetAccountantPenaltyBonus()
...
- FT-12 尚未實作 → 全部回傳 `0`，`effectiveCommissionRate = 0.20`、`effectivePenaltyRate = 0.10`，基本金流仍可運作
- FT-07 尚未實作 → `buildingPenaltyBonus = 0`，不影響正常結算

**有效率範圍**（Jam 階段，單一會計假設）：

- `effectiveCommissionRate ∈ [0.20, 0.22]`（單一會計被動最高 +2%；多會計疊加由 FT-12 cap 控制）
- `effectivePenaltyRate ∈ [0.08, 0.10]`（單一會計指派保險櫃 -2%；多會計疊加最終 cap 至 FT-12 `EFFECT_MAX_*`，下限由 `max(0, ...)` 保證 ≥ 0）
...
| B 難度會計+slot（1 位指派預備金保險櫃） | 600 | 0.08 | 0 | 48 | **-48** |
| A 難度 + 未來 `on_fail_gold_bonus`（**Phase 2 / 未來擴充**） | 800 | 0.10 | 200 | 80 | **+120**（罕見） |
...
> **Phase 2 — Jam 版 no-op**：FT-07 GDD 未定義 `OnGuildMaintenanceDue` 發布時機與維護費表；Jam 版 FT-07 不發布此事件，本管線程式雖實作但永不觸發。FT-12 完成後啟用。
...
> **Phase 2 — Jam 版 no-op**：FT-12 Staff System 於 Jam 版不實作；事件 `OnStaffSalaryDue` 永不發布，本管線程式雖實作但永不觸發。

FT-05 在 `OnEnable` 訂閱 FT-12 `OnStaffSalaryDue(long dueTimestamp, Dictionary<int, int> perStaffSalary, int totalAmount)`。
...
- FT-05 **不計算**薪水金額（由 FT-12 依 `StaffTable` 計算後傳入）
- FT-05 **不判斷**觸發時機（由 FT-12 或 F-02 tick 決定）
- 薪水扣款**可能觸發破產**
...
| `perStaffSalary` | `Dict<int, int>` | `instanceID → salary`，由 FT-12 對 roster 內每個 `StaffInstance` 查 `StaffTable[s.staffID].salary` 組裝；key 為 FT-12 內部 `instanceID`，FT-05 僅 iterate sum 不關心 key 語意 |
| `totalAmount` | `int` | 合計 |

**發布者**：FT-12 Staff System（協同 F-02 Time System 定期 tick）。

> 雙向依賴登記見 § 6；FT-02 / FT-03 / FT-04 / FT-07 / FT-12 GDD 在本系統通過後需補反向聲明。
...
| P-03 Notification | （可選）推播預收通知；`OfflineAutoPick` 併入離線摘要 **【→Log API待更新】** |
...
| P-03 Notification | 依 `isSuccess` + `bankruptcyStateBefore/After` 選擇通知模板（見 3.8 狀態轉移對照表） **【→Log API待更新】** |
...
| P-03 Notification | 依 `bankruptcyStateBefore/After` 選擇通知模板 **【→Log API待更新】** |
...
| `accountantCommissionBonus` | `float` | FT-12 `GetAccountantCommissionBonus()` | `{0, +0.02}` | 會計職員已招募 → `+0.02`；否則 `0` |
| `accountantPenaltyBonus` | `float` | FT-12 `GetAccountantPenaltyBonus()` | `{0, -0.02}` | 會計招募 AND 已指派預備金保險櫃 → `-0.02`；否則 `0` |
| `buildingPenaltyBonus` | `float` | FT-07 `GetBuildingPenaltyBonus()` | `0`（保留擴充） | 建築層額外賠償加成，Jam 未使用 |

**值域**（Jam 階段，假設單一會計）：

- `effectiveCommissionRate ∈ [0.20, 0.22]`（下限：無會計；上限：1 位會計被動 +0.02。多會計疊加 cap 由 FT-12 `EFFECT_MAX_COMMISSION_BONUS` 控制，預設上限可至 `0.30`）
- `effectivePenaltyRate ∈ [0.08, 0.10]`（下限：1 位會計指派預備金保險櫃 -0.02；上限：無加成。多會計疊加可進一步降低，最終由 FT-12 cap 與 `max(0, ...)` 共同保證 ≥ 0）
...
| `totalAmount` | `int` | `OnGuildMaintenanceDue` / `OnStaffSalaryDue` | 上游（FT-07 / FT-12）聚合後傳入 |
...
結果:     P-03 推播「破產警告開始」（含 F-03 倒數秒數）；P-02 金幣變紅 -20 **【→Log API待更新】**
```
...
結果:     P-03 推播「負債已結清」；F-03 停止倒數 **【→Log API待更新】**
```
...
狀態:     bankruptcyStateBefore = Normal, bankruptcyStateAfter = Warning
結果:     P-03 推播「破產警告開始」（維護費變體通知）；P-02 顯示三項設施扣款明細 **【→Log API待更新】**
```
...
狀態:     bankruptcyStateBefore = Normal, bankruptcyStateAfter = Warning
結果:     P-03 推播「破產警告開始」（薪水變體通知）；P-02 顯示兩名職員薪水明細 **【→Log API待更新】**
```
...
結果:     P-03 推播「破產觸發」；FT-06 Guild Core 依 F-03 OnBankruptcyWarningStateChanged 接管遊戲結束流程 **【→Log API待更新】**
...
| FT-07 Guild Building | Event + API | 訂閱 `OnGuildMaintenanceDue`（**Phase 2**，Jam 不發布）；查詢 `GetBuildingPenaltyBonus()`（Jam 預留回傳 `0`） | `null` → `buildingPenaltyBonus = 0`（§3.4）；事件未發 → 管線 no-op |
| FT-12 Staff System | Event + API | 訂閱 `OnStaffSalaryDue`（**Phase 2**，Jam 不發布）；查詢 `GetAccountantCommissionBonus()` / `GetAccountantPenaltyBonus()` | `null` → 兩 bonus 皆 `0`（§3.4）；事件未發 → 管線 no-op |
...
- [x] **FT-12 Staff System** — 已於 FT-12 §6.3 登記為 `OnStaffSalaryDue(long dueTimestamp, Dictionary<int,int> perStaffSalary, int totalAmount)` 發布者；§3.6 / §6.2 提供 `GetAccountantCommissionBonus()` / `GetAccountantPenaltyBonus()` API（系統未解鎖回 0）；內部「會計已招募 AND 指派 slot=5 保險櫃」判斷見 FT-12 §3.6（Effect Aggregation）+ §4.2（cap 公式）
...
3. **FT-05 本體 + FT-04**（成對開發，委託金流可單獨驗證）— 此時 FT-07 / FT-12 尚未實作，`buildingPenaltyBonus = 0`、`accountantBonus = 0`，§4.3 範例 A / B 可完整跑通
4. **FT-12 Staff System**（會計加成生效；§4.3 範例 C 的 `0.22` 傭金率可測；薪水扣款路徑可測）
5. **FT-07 Guild Building**（維護費扣款；§4.3 範例 D 可測）
6. **P-02 / P-03**（UI 與通知；完整玩家體驗）

**降級觀測性**：FT-05 於 `Awake` 檢查依賴注入時，若 FT-07 / FT-12 為 `null`，`Debug.Log("[FT-05] FT-07 not present, buildingPenaltyBonus defaults to 0")` 僅輸出一次，避免每次結算噪音。
...
|------|---------|------|---------|-----------------|
| `ACCOUNTANT_COMMISSION_BONUS` | FT-12 | `+0.02` | `[0, +0.10]` | 疊加至 `effectiveCommissionRate` |
| `ACCOUNTANT_PENALTY_BONUS` | FT-12 | `-0.02` | `[-0.10, 0]` | 疊加至 `effectivePenaltyRate`（配合 `max(0, ...)` 下限） |
...
| `StaffTable.salary` 各職員 | FT-12 | CSV | 視職員 | `OnStaffSalaryDue.totalAmount` |
...
**AC-1 正常預收**
- Given 金幣 = `1000`，FT-12 已招募會計（irrelevant for prepay）
...
**AC-4 成功結算（無加成）**
- Given 金幣 = `1000`，FT-07 / FT-12 null
...
**AC-6 會計 + 保險櫃加成生效**
- Given 金幣 = `1000`，FT-12 回傳 `accountantCommissionBonus = +0.02, accountantPenaltyBonus = -0.02`
- When 成功結算（baseReward=600）→ `commissionGoldAmount = 132`、`effectiveCommissionRate = 0.22`
- When 失敗結算（baseReward=600）→ `penaltyGoldAmount = 48`、`effectivePenaltyRate = 0.08`（公式：`max(0, 0.10 + (-0.02) + 0) = 0.08`，`600 × 0.08 = 48`）
...
**AC-15a MonoBehaviour 訂閱生命週期**
- FT-05 `OnEnable` 訂閱 4 個 In 事件（`OnCommissionAccepted` / `OnMissionResolved` / `OnGuildMaintenanceDue` / `OnStaffSalaryDue`）；`OnDisable` 取消訂閱；重複 enable/disable 後同一事件只處理一次（無 listener leak）

**AC-15b Jam 降級（依賴未實作）**
- FT-07 / FT-12 為 `null` 時，§3.4 null-check 分支回傳 `0f`；AC-4 / AC-5 仍通過（預設 `effectiveCommissionRate = 0.20` / `effectivePenaltyRate = 0.10`，與基礎常數一致）

**AC-15c CSV 資料驅動調整**
- 修改 CSV `SystemConstants.COMMISSION_RATE = 0.30`、重啟後 AC-4 的 `commissionGoldAmount` 由 `120` 變為 `180`；無需改 FT-05 程式碼
