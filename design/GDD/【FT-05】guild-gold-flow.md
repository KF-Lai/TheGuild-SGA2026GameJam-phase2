...
> FT-05 的**委託金流**（預收、結算）於 Jam 版完整實作；**固定支出**（設施維護費 `OnGuildMaintenanceDue`、職員薪水 `OnStaffSalaryDue`）為 **Phase 2** 規格——FT-05 保留完整契約、管線與降級分支（Case 5.4.1），但發布者 FT-07 / FT-08 在 Jam 版**不發布**對應事件，兩條管線因此為 no-op。相關 AC 於 Jam 階段**不驗證實際扣款行為**，僅驗證「無事件輸入 → 無金流、無事件輸出」。
...
> **Phase 2 — Jam 版 no-op**：FT-07 GDD 未定義 `OnGuildMaintenanceDue` 發布時機與維護費表；Jam 版 FT-07 不發布此事件，本管線程式雖實作但永不觸發。FT-08 完成後啟用。
...
> **Phase 2 — Jam 版 no-op**：FT-08 Guild Staff 於 Jam 版不實作；事件 `OnStaffSalaryDue` 永不發布，本管線程式雖實作但永不觸發。
...
| FT-08 Guild Staff | Event + API | 訂閱 `OnStaffSalaryDue`（**Phase 2**，Jam 不發布）；查詢 `GetAccountantCommissionBonus()` / `GetAccountantPenaltyBonus()` | `null` → 兩 bonus 皆 `0`（§3.4）；事件未發 → 管線 no-op |
