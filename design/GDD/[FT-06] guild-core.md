...
對外提供同步查詢 API（`GetRosterCap()` / `GetMaxMissions()` / `GetMaxDifficulty()` / `GetCurrentLevel()`），皆即時讀 `GuildLevelTable` 不維護快取。
...
2. 提供容量查詢 API 給 FT-01（招募名冊上限 + 可招募最高階級）、FT-02（同時派遣上限）；任務難度軸資料型別對齊 C-01
...
`GuildLevelTable`（5 級，欄位：`level`、`reputationThreshold`、`title`、`maxMissions`、`rosterCap`、`maxDifficulty`）
...
- `currentLevel` 從 `GuildLevelTable` 查其他衍生欄位（title、maxMissions、rosterCap、maxDifficulty），**不快取**
...
| level | reputationThreshold | title | maxMissions | rosterCap | maxDifficulty |
|---|---|---|---|---|---|
| 1 | 0 | 新手冒險者公會 | 2 | 4 | D |
| 2 | 100 | 初階冒險者公會 | 3 | 6 | C |
| 3 | 300 | 中階冒險者公會 | 4 | 8 | B |
| 4 | 700 | 高階冒險者公會 | 5 | 10 | A |
| 5 | 1500 | 名聲顯赫的冒險者公會 | 6 | 12 | S |

- 資料源：`Assets/Resources/Data/GuildLevelTable.csv`
- 閾值、稱號將交由 haiku 精修；容量與難度需與 FT-01、FT-02、C-01 對齊
...
| `GetRosterCap()` | `int` | 冒險者上限（FT-01 招募閘） |
| `GetMaxMissions()` | `int` | 同時派遣上限（FT-02 派遣閘） |
| `GetMaxDifficulty()` | `string` | 可接最高難度；字串值域對齊 C-01 難度軸（F/E/D/C/B/A/S/SS/SSS）；目前消費者 = FT-01（老手邀請階級上限） |
...
FOR k = 1 TO (T - C):
    event[k] = OnGuildLevelChanged {
        ...
        newMaxMissions:   GuildLevelTable[C + k].maxMissions
        newRosterCap:     GuildLevelTable[C + k].rosterCap
        newMaxDifficulty: GuildLevelTable[C + k].maxDifficulty
...
| 下游系統 | 訂閱事件 / 查詢 API | 用途 |
|---|---|---|
| **FT-01 Adventurer Recruitment** | `GetRosterCap()`、`GetMaxDifficulty()` | 招募名冊上限閘門 + 老手邀請可招募最高階級 |
| **FT-02 Mission Dispatch** | `GetMaxMissions()` | 同時派遣上限閘門 |
...
- [x] `[FT-01] adventurer-recruitment.md` §6.1：已登記 FT-06（讀 `GetRosterCap()` / `GetMaxDifficulty()`）
- [x] `[FT-02] mission-dispatch.md` §6.1：已登記 FT-06（讀 `GetMaxMissions()`）
- ~~`[C-01] mission-database.md` §6.1：補 FT-06~~ — **不適用**：C-01 為純資料系統（Mission Database），不主動呼叫任何 API；`GetMaxDifficulty()` 的字串值域對齊 C-01 難度軸，但消費者為 FT-01 非 C-01
...
**依賴前置**：F-01 → F-02 → F-03 → **FT-06** → FT-01 / FT-02 / P-01 / P-02 / P-03 / FT-10

- FT-06 自身 API 僅依賴 F-02 / F-03 / F-01，可在 FT-01/FT-02 之前實作
- FT-01、FT-02 容量閘需等 FT-06 `GetRosterCap()` / `GetMaxMissions()` 就緒
...
**檔案位置**：`Assets/Resources/Data/GuildLevelTable.csv`
...
| `maxMissions` | int | 1..10 | 同時派遣上限;下游 FT-02 使用 |
| `rosterCap` | int | 1..20 | 冒險者招募上限；下游 FT-01 使用 |
| `maxDifficulty` | string | 依 C-01 難度軸（F/E/D/C/B/A/S/SS/SSS） | 可接最高難度；下游 FT-01 使用（老手邀請階級上限） |
...
**調整指南**：
- 升級曲線：threshold 差值 = 100, 200, 400, 800（約 2 倍遞增），Jam 階段保守維持
- 難度軸：本表 `maxDifficulty` 值域必須為 C-01 難度軸（F/E/D/C/B/A/S/SS/SSS）的子集；Game Jam 預設 Lv5 上限為 `S`（保留 SS/SSS 給未來擴充）；若 C-01 難度軸改變（如改為數字），本表需同步調整
...
- [ ] `maxMissions`、`rosterCap` 各級單調不減（升級不應縮減容量）
- [ ] `maxDifficulty` 單調不降（難度軸定義內）
...
- **AC-8**：`GetRosterCap()` / `GetMaxMissions()` / `GetMaxDifficulty()` 回傳值與 `GuildLevelTable` 當前等級列完全一致（每次升級後同步反映）
