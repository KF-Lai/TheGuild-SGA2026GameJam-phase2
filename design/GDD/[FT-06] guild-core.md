...
對外提供同步查詢 API（`GetMaxDifficulty()` / `GetCurrentLevel()` / `GetCurrentTitle()` 等），皆即時讀 `GuildLevelTable` 不維護快取。
...
`GuildLevelTable`（5 級，欄位：`level`、`reputationThreshold`、`title`、`maxDifficulty`）
...
| level | reputationThreshold | title | maxDifficulty |
|---|---|---|---|
| 1 | 0 | 新手冒險者公會 | D |
| 2 | 100 | 初階冒險者公會 | C |
| 3 | 300 | 中階冒險者公會 | B |
...
| 下游系統 | 訂閱事件 / 查詢 API | 用途 |
|---|---|---|
| **FT-01 Adventurer Recruitment** | `GetMaxDifficulty()` | 老手邀請可招募最高階級；名冊上限改由 FT-07 `GetRosterCap()` 提供 |
...
- [x] `[FT-01] adventurer-recruitment.md` §6.1：已登記 FT-06（讀 `GetMaxDifficulty()`）；`GetRosterCap()` 來源改為 FT-07
...
- FT-01、FT-02 容量閘需等 FT-07 `GetRosterCap()` / `GetMaxConcurrentMissions()` 就緒；FT-01 老手邀請階級上限需等 FT-06 `GetMaxDifficulty()` 就緒
...
- **AC-8**：`GetMaxDifficulty()` 回傳值與 `GuildLevelTable` 當前等級列完全一致（每次升級後同步反映）；容量類 API（`GetRosterCap` / `GetMaxConcurrentMissions`）已移至 FT-07 驗收
