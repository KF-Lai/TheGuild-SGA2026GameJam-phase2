...
C-03 Profession System 定義遊戲中所有冒險者職業的靜態資料，以 `ProfessionTable` 為核心。Phase 2 起，`ProfessionTable` 作為**「職業」這個 entity 的單一表**：除既有的擅長類型（`strongTypeIDs`）/ 弱點類型（`weakTypeIDs`）/ 職業階層（`tier`）/ 升級來源（`baseProfessionID`）外，**並列承載職業 → 種族隨機池欄位（`raceIDs` / `raceWeights`，C-04 消費）與職業 → 特質群組欄位（`traitGroupIDs`，C-05 消費）**——以同 PK = `professionID`（7 行）的 1:1 自然關係收斂掉原 `ProfessionRacePool.csv` / `ProfessionTraitPool.csv` 兩張小表。架構在 F-01 DataManager 之上，提供職業查詢 API，並作為 C-04 Race System / C-05 Trait System 隨機池的引用依據。FT-02 Mission Dispatch 查詢職業的擅長/弱點以套用成功率修正。系統設計為完全資料驅動：新增職業只需在 CSV 加行，所有職業相關欄位同表編輯，程式碼無需改動。Profession System 不持有任何 runtime 狀態。
...
| `raceIDs`（C-04 消費）| `int[]`（`\|` 分隔） | 該職業冒險者實例化時的可抽種族 ID 列表（C-04 §3.2 消費；C-04 透過 `C-03.GetProfession(professionID).raceIDs` 讀取） |
| `raceWeights`（C-04 消費）| `int[]`（`\|` 分隔） | 對應每個 `raceIDs` 的相對權重；長度必須與 `raceIDs` 一致 |
| `traitGroupIDs`（C-05 消費）| `int[]`（`\|` 分隔） | 該職業冒險者實例化時抽取的特質群組 ID 列表（C-05 §3.4 消費；C-05 透過 `C-03.GetProfession(professionID).traitGroupIDs` 讀取） |
...
| professionID | name | strongTypeIDs | weakTypeIDs | tier | baseProfessionID | raceIDs | raceWeights | traitGroupIDs |
|---|---|---|---|---|---|---|---|---|
| 1 | 戰士 | 1 | 4 | 1 | 0 | 1\|3 | 60\|40 | 1\|3\|4 |
| 2 | 法師 | 4 | 3 | 1 | 0 | 2\|4 | 55\|45 | 1\|2\|4 |
| 3 | 遊俠 | 3 | 2 | 1 | 0 | 1\|2 | 50\|50 | 1\|2\|5 |
| 4 | 斥侯 | 2\|4 | 1 | 1 | 0 | 2\|1 | 60\|40 | 1\|2\|4 |
| 5 | 盾衛 | 2 | 4 | 1 | 0 | 1\|3 | 70\|30 | 1\|3\|5 |
| 6 | 治癒師 | 2 | 1 | 1 | 0 | 2\|1 | 60\|40 | 1\|2\|5 |
| 7 | 傭兵 | 0 | 0 | 1 | 0 | 1\|3\|4 | 50\|30\|20 | 2\|4\|5 |

> 種族池與特質群組的設計意圖見 C-04 §3.2 / C-05 §3.4 對應行。
...
| C-04 Race System | 種族隨機池欄位（`raceIDs` / `raceWeights`）已合併入 ProfessionTable；C-04 §3.2 透過此 API 讀取後執行加權抽取 | `GetProfession(professionID)` 取 `raceIDs` / `raceWeights` |
| C-05 Trait System | 特質群組欄位（`traitGroupIDs`）已合併入 ProfessionTable；C-05 §3.4 透過此 API 讀取後遍歷群組 | `GetProfession(professionID)` 取 `traitGroupIDs` |
...
- 新增職業：在 CSV 加行，分配唯一 `professionID`，**同時填妥 `raceIDs` / `raceWeights` / `traitGroupIDs` 欄位**（程式碼無需更動）
- 調整擅長/弱點：直接修改 `strongTypeIDs`/`weakTypeIDs` 欄位
- 調整種族池 / 特質群組：與 C-04 / C-05 owner 同步意圖後直接修改欄位（C-04 / C-05 仍負責安全範圍評估）
