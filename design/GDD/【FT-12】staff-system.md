...
_建立時間：2026-04-26_
_狀態：草稿（從 FT-08 拆出，已套用 design-review 修訂 v2，待二次 review）_
_最後更新：2026-04-27（design-review v2 修訂：B-1~B-8 / M-1~M-8 / N-1~N-5）_
_系統 ID：FT-12_
...
- 依賴 GDD 約束：
  ...
  - `[FT-06] guild-core.md` — 無直接 runtime 依賴（minGuildLevel 過濾在 FT-08 gacha 端，FT-12 不消費 GetCurrentLevel）
  - `[F-02] time-system.md` — `OnDailyReset` 驅動薪水管線（M-2 案，Phase 2）；`OnMinuteTick` + 自節流（每 3600s 一次）驅動自動轉休假檢查（F-02 無 OnHourTick）
...
> **Phase 2 註記**：薪水管線完整設計保留於 §3.9 / §4.2 / §5.4 / §8.7，**Jam 版不實作**——對齊 FT-05 既定 Phase 2 立場（FT-05 §3.5 / §6.7）。觸發事件為 `OnDailyReset`（M-2 案），非 `OnHourTick`。
...
StaffInstance {                                      // ↓ 持久化欄位（FT-10 序列化）
    instanceID                         : int         // 唯一識別碼；遞增 int（FT-12 維護 _nextInstanceID counter）
...
- **`instanceID`**：FT-12 維護遞增 counter `_nextInstanceID`，新遊戲起始 `1`，每次 HireStaff 成功後 +1；同存檔內永不重複、永不回收（解雇也不釋出）。
...
- **`assignedBuildingID` 跨狀態語意**：
  - 再分配中 `assignedBuildingID == 0`（B-5 案：Reallocating 表示「未指派任一建築、等待玩家選 buildingID」，不代表目標 slot，§3.4 詳述）
  - 進入休假時清為 `0`
  - 工作中代表**目前佔用**的 slot，必然 `> 0`（B-9 A 案：無 slot 能力職員永不進 Working，其 Reallocating 為穩態，§3.7.5）
...
| `minGuildLevel` | int | 可錄用最低公會等級（FT-08 gacha 池過濾條件之一；Jam 版範圍 1~5，§7.1.1）|
...
| `AccountantCommission` | **Passive**（M-1 案）| `GetAccountantCommissionBonus` | 職員狀態 ∈ { Working, Reallocating }（無需指派至保險櫃） | float 比率（如 `+0.02`） |
...
| staffID | name | rarity | salary | severancePay | isFiller | factionID | minGuildLevel | effectIDs | effectValues | slotBuildingIDs | uiFlagIDs | uiFlagBuildingIDs |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 101 | 克勞德·會計師 | 3 | 40 | 120 | false | 0 | 2 | `AccountantCommission,AccountantPenaltyOnVault` | `0.02,-0.02` | `5` | *(empty)* | *(empty)* |
| 102 | 艾蓮·委託官 | 2 | 30 | 80 | false | 0 | 1 | `Willingness` | `0.05` | `1` | `SuccessRatePreview` | `1` |
| 103 | 蘿絲·櫃台小姐 | 2 | 30 | 80 | false | 0 | 1 | `RecruitRefreshOnCounter` | `7200` | `4` | *(empty)* | *(empty)* |
| 104 | 無名小職員 | 1 | 10 | 10 | true | 0 | 1 | `Willingness` | `0.01` | `0` | *(empty)* | *(empty)* |
| 105 | 咖啡杯 | 1 | 0 | 0 | true | 0 | 1 | *(empty)* | *(empty)* | `0` | *(empty)* | *(empty)* |
...
HireStaff(CandidateCard candidate) → HireResult:
    ...
    Step 3: 建立 StaffInstance（§3.1，B-9 A 案：依 slot 能力分流 reallocatingStart）
        tbl = StaffTable[candidate.staffID]
        hasSlot = tbl.slotBuildingIDs.Any(id => id > 0)
        instance = new StaffInstance {
            instanceID:                         _nextInstanceID++,   // 全域單調遞增
            staffID:                            candidate.staffID,
            currentState:                       Reallocating,          // 一律 Reallocating（B-5 案）
            assignedBuildingID:                 0,
            buildingSwitchCooldownEndTimestamp: 0,
            reallocatingStartTimestamp:         hasSlot ? now : 0,     // 有 slot 能力才計 auto-leave；無 slot 能力為穩態
            hiredTimestamp:                     now
        }
        // 註：無 slot 能力職員（slotBuildingIDs 為空或全 0）也建立為 Reallocating，
        // 但 reallocatingStart = 0；§3.7 auto-leave 不適用（HasSlotCapability=false），
        // Reallocating 為穩態，passive effect 永久生效。
        // 欄位不變式：reallocatingStart > 0 ⟺ 正在計時 auto-leave（§3.6.5）
...
- **入職即 Reallocating**（B-5 案）：對齊 §3.6 三態定義—— Working 必須由「進入 Working」transition 觸發（並依 §3.6.4 設冷卻），新建 instance 一律 Reallocating；玩家透過 `TryAssignStaff(buildingID)` 指派至建築進入 Working
- **`reallocatingStartTimestamp = now`**：§3.7 自動轉休假倒數從入職瞬間開始；若玩家錄用後超過 `REALLOCATING_AUTO_LEAVE_SECONDS` 未指派且該職員具 slot 能力，職員自動轉 OnLeave。**無 slot 能力職員例外**（§3.7.5）
- **不扣費**：招聘費 / 刷新費由 FT-08 在 TryRecruit 階段扣（FT-08 §3.3.4）；FT-12 入職階段不再扣費
...
> **欄位歸屬說明（B-1 案 / 2026-04-27 修訂）**：`effectIDs` / `effectValues` / `uiFlagIDs` / `uiFlagBuildingIDs` 屬 **StaffTable**（§3.2 schema 模板層），**不**屬 StaffInstance（§3.1 runtime 層）。聚合演算法每次遍歷 roster 時透過 `StaffTable[staff.staffID]` 查靜態資料；不複製到 instance（避免序列化體積膨脹、利於資料表熱更）。下文演算法以 `tbl = StaffTable[staff.staffID]` 簡寫該查詢。

FT-12 對外提供 **5 個查詢 API**，分兩組：
...
```csharp
int  GetRecruitRefreshReductionSec();   // 消費者：FT-08 Gacha System（auto refresh 間隔減免）
...
GetStaffWillingnessBonus():
    ...
    FOR each staff in activeRoster:
        ...
        tbl = StaffTable[staff.staffID]                  // §3.4 開頭：靜態資料查模板
        idx = indexOf(Willingness, tbl.effectIDs)
        IF idx >= 0:
            sum += tbl.effectValues[idx]
...
GetAccountantCommissionBonus():
    ...
    FOR each staff in activeRoster:
        ...
        tbl = StaffTable[staff.staffID]
        idx = indexOf(AccountantCommission, tbl.effectIDs)
        IF idx >= 0:
            sum += tbl.effectValues[idx]
...
GetAccountantPenaltyBonus():
    ...
    FOR each staff in activeRoster:
        ...
        tbl = StaffTable[staff.staffID]
        idx = indexOf(AccountantPenaltyOnVault, tbl.effectIDs)
        IF idx >= 0 AND staff.assignedBuildingID == 5:    // 已指派預備金保險櫃
            sum += tbl.effectValues[idx]                  // effectValue 為負（如 -0.02）
...
GetRecruitRefreshReductionSec():
    ...
    FOR each staff in activeRoster:
        IF staff.currentState != Working: continue         // Slot effect 僅工作中生效
        tbl = StaffTable[staff.staffID]
        idx = indexOf(RecruitRefreshOnCounter, tbl.effectIDs)
        IF idx >= 0 AND staff.assignedBuildingID == 4:     // 已指派公會櫃臺
            sum += (int)tbl.effectValues[idx]              // 單位秒（如 7200 = 2h）
...
- 符合 FT-03 §5 / FT-05 §3.4「FT-12 名冊空 / 系統未解鎖 → 回 0」的降級契約
...
IsSuccessRatePreviewEnabled() → bool:
    ...
    FOR each staff in activeRoster:
        ...
        tbl = StaffTable[staff.staffID]                    // 靜態資料查模板（§3.4 開頭）
        idx = indexOf(SuccessRatePreview, tbl.uiFlagIDs)
        IF idx >= 0 AND staff.assignedBuildingID == tbl.uiFlagBuildingIDs[idx]:
...
| 切換建築冷卻 | `StaffInstance.buildingSwitchCooldownEndTimestamp` | UTC 秒；冷卻僅由「進入 Working」transition 觸發（B-5 案）：`cooldownEnd ← now + BUILDING_SWITCH_COOLDOWN_SECONDS`（§3.6.4）|
...
TryAssignStaff(instanceID, buildingID):
    ...
    1.5 入口轉假掃描：CheckReallocatingAutoLeave()         // §3.7；先處理逾時轉假再讀 staff state
    2. 找職員：staff = roster.Find(instanceID)；找不到 → STAFF_NOT_FOUND
    3. 合格性：
       tbl = StaffTable[staff.staffID]
       buildingID ∉ tbl.slotBuildingIDs → BUILDING_NOT_ELIGIBLE

    4. Idempotent 短路（B-6 案：先於冷卻檢查）：
       IF staff.currentState == Working AND staff.assignedBuildingID == buildingID:
           return SUCCESS                                  // no-op、不發事件、不查冷卻
       // 註：Reallocating + 同 buildingID 不可能（Reallocating 不變式：assignedBuildingID == 0）

    5. 狀態檢查：staff.currentState == OnLeave → STAFF_ON_LEAVE
       // OnLeave → Working 必須先 TryReturnFromLeave 進 Reallocating

    6. 冷卻檢查（B-5 案：冷卻擋住「進入 Working」transition）：
       IF now < staff.buildingSwitchCooldownEndTimestamp → SWITCH_COOLDOWN
    ...
    7. Capacity 檢查：
       ...
       // 註：step 4 已排除「同建築 idempotent」，本檢查不需 (assignedBuildingID != buildingID) 例外

    8. 執行指派（B-5 案：所有路徑皆「進入 Working」、皆設冷卻）：
       ...
       staff.reallocatingStartTimestamp = 0                // 進 Working 重置 Reallocating 計時
...
> **Reallocating 設計意圖（A2 案 / B-5 修訂 2026-04-27）**：Reallocating **僅**表示「職員已不在任何建築工作、等待玩家選 buildingID 落位」（`assignedBuildingID == 0` 為其不變式）；**不**用於切建築過渡。冷卻語意改為「進入 Working」transition 觸發（B-5 案，§3.6.4）；無論 Reallocating → Working 或 Working → Working 切建築，皆觸發冷卻 `BUILDING_SWITCH_COOLDOWN_SECONDS`。
>
> **Reallocating 三大進入路徑**（皆 `assignedBuildingID = 0`、不觸發冷卻）：
> 1. **錄用後等待**（§3.3.2）：一律 Reallocating；具 slot 能力職員須 TryAssignStaff(X) 落位 Working；無 slot 能力職員 Reallocating 為穩態（§3.7.5）
> 2. **取消指派**：Working → TryUnassignStaff → Reallocating（**不觸發冷卻**，B-5 新語意）；玩家可立即 TryAssignStaff(X)，但該次「進入 Working」會觸發冷卻
> 3. **從休假回崗**：OnLeave → TryReturnFromLeave → Reallocating（**不觸發冷卻**）；玩家須 TryAssignStaff(X) 落位 Working（觸發冷卻）
>
> **切建築直接 Working → Working**：Working @A → TryAssignStaff(B) → Working @B（state 不變、assignedBuildingID 改、`cooldownEnd ← now + BUILDING_SWITCH_COOLDOWN_SECONDS`）；slot effects 在新建築立即生效；冷卻期間任何「進入 Working」transition 皆被 SWITCH_COOLDOWN 擋下。
>
> **冷卻新語意（B-5）**：冷卻僅由「進入 Working」transition 觸發，不由其他切換觸發。冷卻中允許 `Working ↔ Reallocating ↔ OnLeave` 之間自由切換（這些不算「進入 Working」），但任何「進入 Working」一律被擋。詳見 §3.6.4。
>
> **Auto-leave 適用範圍**（§3.7）：僅對「具 slot 能力 + 處於 Reallocating + reallocatingStartTimestamp 超 `REALLOCATING_AUTO_LEAVE_SECONDS`」的職員計時；無 slot 能力職員與 Working 職員不計時。
...
TryUnassignStaff(instanceID):
    ...
    4. 執行取消指派（B-5 案：離開 Working 不觸發冷卻）：
       oldBuildingID = staff.assignedBuildingID
       oldState      = staff.currentState
       staff.assignedBuildingID = 0
       staff.currentState = Reallocating
       staff.reallocatingStartTimestamp = now
       // 不重置 buildingSwitchCooldownEndTimestamp；既有冷卻沿用倒數
       // 註：無繞過風險——下次 TryAssignStaff(X) 進 Working 仍會觸發新冷卻（§3.5.3 step 8）
...
當 `tbl.slotBuildingIDs.Count > 1` 時（例：可選櫃台或保險櫃），玩家透過 P-02 UI 選擇實際指派位置；FT-12 不提供「自動最佳建築」邏輯（避免設計黑箱、玩家失控）。P-02 UX 細節見 A.4 對 P-02 的雙向聲明。

#### 3.5.6 Effect 聚合的指派時機影響

| 階段 | Slot effect 生效 | Passive effect 生效 |
|---|---|---|
| Reallocating（`assignedBuildingID = 0`、等待落位 / 無 slot 能力穩態）| ❌ | ✅ |
| Working、`assignedBuildingID` 符合 effect 條件 | ✅ | ✅ |
| Working、`assignedBuildingID` 不符合 effect 條件 | ❌ | ✅ |
| OnLeave | ❌ | ❌ |

**重新聚合時機**：FT-12 不快取聚合結果（§3.4 演算法每次調用即時遍歷 roster），因此 `OnStaffAssigned` / `OnStaffStateChanged` 事件不需要主動觸發 cache invalidation；下游（FT-08 / FT-03 / FT-05）按需 query 即可拿到最新值。
...
| Reallocating 是否占 capacity？ | **不占**（Reallocating 必然 `assignedBuildingID == 0`，自然不對應任何 buildingID 的 capacity）|
| OnLeave 是否占 capacity？ | **不占**（同上）|
| 同 staff 重複 TryAssignStaff 同 buildingID | idempotent（B-6：短路在冷卻檢查前）；no-op、不發事件、不查冷卻、回 SUCCESS |
...
| State | 語意 | Passive Effect | Slot Effect | 領薪水（Phase 2）|
|---|---|---|---|---|
| `Working` | 在指派的建築工作中（assignedBuildingID > 0；無 slot 能力職員除外，其 Reallocating 為穩態，永不進 Working）| ✅ | ✅（assignedBuildingID 符合）| ✅ |
| `Reallocating` | 未指派任一建築、等待玩家選 buildingID（assignedBuildingID == 0；入口：錄用後 / 取消指派 / 從休假回崗 / 無 slot 能力職員穩態）| ✅ | ❌ | ✅ |
| `OnLeave` | 休假中 | ❌ | ❌ | ❌（§4.2.1 排除）|

#### 3.6.2 狀態轉移總表（B-5 修訂）

| 起始狀態 | 終止狀態 | 觸發 | 進入 Working？ | 副作用 |
|---|---|---|---|---|
| (錄用) | `Reallocating` | `HireStaff` 成功 | ❌ | state = Reallocating、assignedBuildingID = 0；**具 slot 能力**職員 reallocatingStart = now、**無 slot 能力**職員 reallocatingStart = 0（B-9 A，§3.3.2 / §3.7.5）|
| `Reallocating` | `Working` | `TryAssignStaff(buildingID)` 落位 | ✅ **觸發冷卻** | assignedBuildingID = buildingID、reallocatingStart = 0、`cooldownEnd ← now + BUILDING_SWITCH_COOLDOWN_SECONDS` |
| `Working` | `Working` | `TryAssignStaff(otherBuildingID)` 切建築 | ✅ **觸發冷卻** | assignedBuildingID = otherBuildingID、`cooldownEnd ← now + BUILDING_SWITCH_COOLDOWN_SECONDS` |
| `Working` | `Reallocating` | `TryUnassignStaff` | ❌ | assignedBuildingID = 0、reallocatingStart = now、cooldown 不重置 |
| `Reallocating` | `OnLeave` | `now − reallocatingStart ≥ REALLOCATING_AUTO_LEAVE_SECONDS` 且具 slot 能力（§3.7）| ❌ | assignedBuildingID = 0、reallocatingStart = 0、cooldown 不重置 |
| `Working` | `OnLeave` | `TryGoOnLeave(instanceID)` | ❌ | assignedBuildingID = 0、reallocatingStart = 0、cooldown 不重置（M-4：發 OnStaffAssigned + OnStaffStateChanged）|
| `Reallocating` | `OnLeave` | `TryGoOnLeave(instanceID)` | ❌ | 同上（無 OnStaffAssigned 因 oldBuildingID 已是 0）|
| `OnLeave` | `Reallocating` | `TryReturnFromLeave(instanceID)` | ❌ | reallocatingStart = now、cooldown 不重置（玩家須在 REALLOCATING_AUTO_LEAVE_SECONDS 內 TryAssignStaff，否則自動轉假）|
| 任何 | (銷毀) | `TryFireStaff(instanceID)` | — | StaffInstance 從 roster 移除（§3.8）|

> **冷卻新語意（B-5 案 / 2026-04-27 修訂）**：冷卻**僅由「進入 Working」transition 觸發**。觸發場景共三類：`Reallocating → Working`、`Working → Working`（切建築）、`OnLeave → Working`（不存在直接轉移；OnLeave 必須先 TryReturnFromLeave 進 Reallocating，再 TryAssignStaff 才能進 Working，冷卻由後者觸發）。冷卻**不由其他切換觸發**——`Working → Reallocating`（unassign）、`Working → OnLeave`、`Reallocating → OnLeave`、`OnLeave → Reallocating` 皆不重置冷卻。
>
> **冷卻 gate（B-5）**：冷卻中（`now < buildingSwitchCooldownEndTimestamp`）任何「進入 Working」transition 一律被擋（TryAssignStaff 回 SWITCH_COOLDOWN）；冷卻中允許 `Working ↔ Reallocating ↔ OnLeave` 自由切換、保持 Working 不受影響。
>
> **設計目的**：阻止玩家頻繁切換建築（每次切換要等 `BUILDING_SWITCH_COOLDOWN_SECONDS = 21600s = 6h`）；同時不限制玩家 unassign / 休假 / 回崗 等狀態切換（這些不算「換工作」）。
>
> **Reallocating 入口路徑**（皆 `assignedBuildingID = 0`、皆**不**觸發冷卻）：1) 錄用後（§3.3.2）；2) 取消指派（§3.5.4）；3) 從休假回崗（TryReturnFromLeave）。
>
> **Reallocating 計時器**：`reallocatingStartTimestamp` 於進入 Reallocating 時設為 now；於進入 Working（落位）或 OnLeave（休假）時 reset 為 0。
...
`buildingSwitchCooldownEndTimestamp` 與 `currentState` 正交（§3.1 已述）。冷卻僅由「進入 Working」transition 觸發、其他切換不重置：

| Transition | 冷卻處理 |
|---|---|
| `Reallocating → Working`（落位）| `cooldownEnd ← now + BUILDING_SWITCH_COOLDOWN_SECONDS` ✅ |
| `Working → Working`（切建築）| `cooldownEnd ← now + BUILDING_SWITCH_COOLDOWN_SECONDS` ✅ |
| `Working → Reallocating`（取消指派）| 不重置（沿用既有倒數）|
| `Working → OnLeave`（休假）| 不重置 |
| `Reallocating → OnLeave`（休假 / 自動轉假，§3.7）| 不重置 |
| `OnLeave → Reallocating`（回崗）| 不重置 |
| `Reallocating ↔ Reallocating`（保持）| 不重置 |
| 跨 session 載入 | 冷卻照舊（時間戳持久化）|

**冷卻 gate**：冷卻中（`now < buildingSwitchCooldownEndTimestamp`）TryAssignStaff 一律回 `SWITCH_COOLDOWN`（不論 from state 為 Reallocating / Working）；其他切換 API（TryUnassignStaff / TryGoOnLeave / TryReturnFromLeave）不查冷卻、不受冷卻影響。

**設計理由**：冷卻針對「玩家頻繁切換建築操弄聚合結果」；新語意聚焦於「進入 Working」這個唯一語意點——任何進入 Working 都受 6h 限制，不論前一狀態為何。其他切換（unassign / 休假 / 回崗）不算「換工作」，不需冷卻；玩家可自由用這些操作組合，但下次「進入 Working」仍受冷卻約束，無繞過空間。
...
| `assignedBuildingID` | ✅ | OnLeave → 0；Reallocating → 0（B-5 不變式）；Working → ≥ 1（必須佔用某 slot；Working + 0 為非法狀態 §6.7 step 3 reset 為 Reallocating）|
| `reallocatingStartTimestamp` | ✅ | state == Reallocating 且具 slot 能力 → > 0；其他（Working / OnLeave / 無 slot 能力 Reallocating） → 0 |
| `buildingSwitchCooldownEndTimestamp` | ✅ | ≥ 0 |
...
//   - reallocatingRemainingSec：
//       state == Reallocating 且 reallocatingStart > 0（具 slot 能力計時中）：
//           max(0, reallocatingStart + REALLOCATING_AUTO_LEAVE_SECONDS − now)
//       其他（state ≠ Reallocating；或無 slot 能力穩態 reallocatingStart == 0）：0
...
避免玩家「錄用後 / unassign 後 / 從休假回崗後忘記選 buildingID」造成 Reallocating 狀態無限延伸；強制 `REALLOCATING_AUTO_LEAVE_SECONDS` 後自動轉 OnLeave，使玩家必須主動回崗（TryReturnFromLeave + TryAssignStaff）才能繼續使用該職員的 Slot effect。

> B-5 案（2026-04-27 修訂）下，切建築走 `Working → Working`（state 不變），因此本節規則只對 Reallocating 職員適用；同時對「無 slot 能力職員」（slotBuildingIDs 空或全 0）例外（§3.7.5），其 Reallocating 為穩態。
...
shouldAutoLeave(staff) =
    ...
    AND HasSlotCapability(staff)                          // 無 slot 能力職員例外（§3.7.5）

HasSlotCapability(staff) =
    LET tbl = StaffTable[staff.staffID]
    RETURN tbl.slotBuildingIDs.Any(id => id > 0)          // 至少一個合法 buildingID
```
...
- **OnMinuteTick + 自節流每 3600s 一次**（FT-02 訂閱）：線上每小時掃描一次（F-02 無原生 OnHourTick，FT-12 訂閱 OnMinuteTick 並用 `_lastAutoLeaveScanTimestamp` 自行節流）
- **所有 Try* API 入口**（N-3 入口總表）：

| API | step 1.5 是否掃描 |
|---|---|
| `HireStaff` | 否（新建 instance、不影響既有 roster）|
| `TryAssignStaff` | ✅ |
| `TryUnassignStaff` | ✅ |
| `TryGoOnLeave` | ✅ |
| `TryReturnFromLeave` | ✅ |
...
設計目的：避免 race condition（玩家剛要操作就觸發轉假，造成回傳碼不一致）；HireStaff 例外因新 instance 不需 auto-leave 影響。

#### 3.7.4 與冷卻的互動

`buildingSwitchCooldownEndTimestamp` **不**因自動轉假重置（§3.6.4）。玩家被自動轉假後 TryReturnFromLeave 回 Reallocating → 仍受原冷卻約束（直到 cooldown 結束才能 TryAssignStaff 進 Working）。

#### 3.7.5 邊緣情境（B-5 / M-3 修訂）

| 情境 | 行為 |
|---|---|
| 玩家 unassign → 超過 REALLOCATING_AUTO_LEAVE_SECONDS 仍未指派 | 自動轉 OnLeave（cooldown 不重置）|
| 玩家 assign 切建築（B-5 案，Working → Working）| 不進 Reallocating；不受 auto-leave 影響（state 已為 Working）|
| 玩家在 REALLOCATING_AUTO_LEAVE_SECONDS - 1s 時 TryAssignStaff 落位 | Reallocating → Working、不轉假、reallocatingStart = 0、cooldown 觸發 |
| 玩家離線跨 REALLOCATING_AUTO_LEAVE_SECONDS | OnStaffSystemBoot 時批次轉假所有跨閾值的具 slot 能力職員 |
| **無 slot 能力職員 Reallocating**（slotBuildingIDs 空或全 0）| **不轉假**——HasSlotCapability(staff) == false 時 shouldAutoLeave 永為 false；Reallocating 為穩態，passive effect 永久生效 |
| 系統降級期間 | CheckReallocatingAutoLeave 不執行；解鎖時為跨閾值職員 reset reallocatingStartTimestamp ← now（M-3 案，§3.10.3）|
| 系統降級恰好跨閾值 | 解鎖時 reset reallocatingStart ← now，重新計時；不立刻批次轉假 |
...
TryFireStaff(instanceID):
    1. 系統閘：IsStaffSystemUnlocked() == false → STAFF_SYSTEM_LOCKED
    1.5 入口轉假掃描：CheckReallocatingAutoLeave()      // §3.7；解雇 OnLeave 職員不影響流程，但維持入口契約一致（§3.7.3）
    2. 找職員：staff = roster.Find(instanceID)；找不到 → STAFF_NOT_FOUND
    3. 計算資遣費（M-7 案：純由 StaffTable 控制，無 base 加成）：
...
**設計決策（不可調）**：發布 `OnStaffFired` **先於** `roster.Remove(staff)`。
...
> **訂閱者注意（N-5）**：在 OnStaffFired callback 內呼叫 `Get*Bonus()` 等聚合 API 時，被解雇職員**仍在 roster 中**（事件先於 Remove 發布），聚合結果包含該職員。若需「解雇後」的聚合值，請於下一 frame 或下一個 EventBus tick 重新 query。
...
| 解雇 Working 職員（B-7 案）| 允許；**直接銷毀，不執行 unassign 流程**；只發 OnStaffFired，不發 OnStaffAssigned / OnStaffStateChanged；資遣費照付；effect 立即停用（§3.4 即時遍歷下次 query 即無此 instance）|
...
>
> 對齊 FT-05 既有 Phase 2 立場。Jam 版 FT-12 **薪水管線整段不啟用**——不訂閱 `OnDailyReset`（M-2 案）以觸發薪水、不執行 OnSalaryTick / ProcessOfflineSalary、永不發布 `OnStaffSalaryDue`。職員「招募即終身免費」為已知 Jam 範圍妥協；OnStaffSystemBoot Step D 直接 skip。
>
> 註：`OnDailyReset` 與 `OnMinuteTick` 為 F-02 兩個獨立事件——前者驅動薪水（Phase 2，FT-12 Jam 版不訂閱），後者驅動 auto-leave 掃描（§3.7，FT-12 Jam 版訂閱並自節流為每 3600s 一次）；兩者 Jam 版獨立啟停。
>
> 下列 §3.9.1 ~ §3.9.8 內容**保留為 Phase 2 規格**，Codex 工項書階段全部跳過實作；Phase 2 啟用時直接套用本節，並同步移除 FT-05 的 Phase 2 註記（FT-05 §3.5 / §6 / §7）。

#### 3.9.1 觸發時點（Phase 2，M-2 案）

- **線上 tick**：FT-12 訂閱 F-02 TimeSystem 的 `OnDailyReset` 事件，每次觸發呼叫 `OnSalaryTick(now)`；不需自行過濾 hour
- **離線啟動**：OnStaffSystemBoot Step D 補發跨日累積薪水（§3.9.4）

> M-2 修訂：原設計用 `OnHourTick + UTC 06:00 過濾`，M-2 案改為直接訂閱 `OnDailyReset`（F-02 既有事件），由 F-02 統一定義「每日結算時點」。Phase 2 啟用時直接訂閱即可，無需重複實作 hour 過濾邏輯。

#### 3.9.2 perStaffSalary 組裝（與 §4.2.1 對齊）

```
AssemblePerStaffSalary():
    perStaffSalary = {}
    rosterSnapshot = roster.ToArray()                 // 原子 snapshot（§3.9.2 snapshot 語意）
...
OnSalaryTick(dueTimestamp):
    1. 系統閘：IsStaffSystemUnlocked() == false → return（§3.10）
    2. 時鐘檢查：now < lastSalaryTimestamp → return（§5.1 時鐘倒退）
...
ProcessOfflineSalary():
    ...
    IF lastSalaryTimestamp == 0:
        ...
        return
    IF now < lastSalaryTimestamp: return              // 時鐘倒退（§5.1）
...
| 重新解鎖（L0 → L1） | `lastSalaryTimestamp ← now`（防補發降級期間薪水，§3.9.7）|
| 時鐘倒退 | 不更新 |

#### 3.9.6 OnLeave 職員薪水跳過

§4.2.1 / §3.9.2 已定義：OnLeave 職員不入 perStaffSalary。**特殊情境**：

- 發薪當 frame 玩家 `TryGoOnLeave(instanceID)` → 視 snapshot 時點：snapshot 在 TryGoOnLeave 之前 → 計薪；之後 → 不計薪
- **設計上不保證**這微秒級的順序；玩家不應依賴此行為（§3.9.2 已述用 atomic snapshot 防 dict 污染）

#### 3.9.7 系統降級時的薪水管線

```
OnStaffSystemUnlocked():    // 由 §3.10.5 訂閱 OnBuildingUpgraded 觸發（Phase 2 才生效）
    lastSalaryTimestamp ← now                          // M-8 案：無條件 reset；首次解鎖也設 now，防補發降級期間薪水
```

> M-8 修訂：原條件 reset（`lastSalaryTimestamp != 0` 才 reset）與 AC-2 / §5.1 / §3.10.5 「解鎖瞬間設 now」要求矛盾；新邏輯統一為**無條件 reset**——首次解鎖（lastSalaryTimestamp == 0）與後續解鎖（!= 0）皆設為 now，語意一致且不補發歷史薪水。註：此 callback 為 **Phase 2 啟用後才訂閱**；Jam 版整段 §3.9 不執行，本 callback 不存在。
...
| `CheckReallocatingAutoLeave`（§3.7） | 降級期間不執行；**解鎖時為跨閾值具 slot 能力職員 reset `reallocatingStartTimestamp ← now`**（M-3 案，§3.7.5）|
| `OnSalaryTick`（OnDailyReset 訂閱，M-2 案）| 早退；**Jam 範疇下未訂閱 OnDailyReset（§3.9 Phase 2），不存在觸發路徑** |
...
- `_lastSalaryTimestamp`：Jam 版完全保留不變（§3.9 Phase 2 不訂閱解鎖 callback）；Phase 2 啟用後依 §3.9.7 解鎖時 reset 為 now
...
| TryGoOnLeave（M-4 案）| 1. `oldBuildingID = staff.assignedBuildingID`（快照）→ 2. `oldState = staff.currentState` → 3. `staff.currentState = OnLeave`、`staff.assignedBuildingID = 0`、`staff.reallocatingStartTimestamp = 0` → 4. **IF oldBuildingID > 0**: Publish `OnStaffAssigned { instanceID, oldBuildingID, newBuildingID = 0 }` → 5. Publish `OnStaffStateChanged { instanceID, oldState, newState = OnLeave }` |
...
| OnStaffAssigned | P-02 | 名冊 UI 更新指派狀態圖示（觸發點：TryAssignStaff / TryUnassignStaff / TryGoOnLeave 從 Working 進 OnLeave）|
...
| `Σ_i AccountantCommissionEffectValues[i]` | 同上、`AccountantCommission`（M-1 Passive，無需指派保險櫃）| 同上 |
...
>
> 對齊 §3.9 / FT-05 既有 Phase 2 立場。下列公式（每日薪水 dict 組裝、總額、離線補發）保留為 Phase 2 規格、Jam 版 Codex 不需實作；其引用之 `lastSalaryTimestamp`、`OFFLINE_MAX_SECONDS` 皆無 runtime 觸發路徑（M-2 案：觸發事件改為 OnDailyReset，移除 SALARY_UTC_HOUR 概念）。Phase 2 啟用時直接套用本節，並同步移除 FT-05 §3.5 的 Phase 2 註記。

#### 4.2.1 每日薪水 dict 組裝（Phase 2）

每日 `OnDailyReset` 觸發時（M-2 案，由 F-02 §6.3 定義時點），組裝 `perStaffSalary: Dict<int, int>`（key = `instanceID`、value = 薪水）：
...
每一個補發週期獨立組裝 `perStaffSalary` 一次、發布事件一次（逐次觸發 FT-05 扣款；具體離線迴圈邏輯於 §3.9.4 定義）。
...
| `reallocatingStartTimestamp == 0`（舊存檔 / 無 slot 能力穩態）| `CheckReallocatingAutoLeave` 跳過該 instance；下次 `TryAssignStaff` 落位 Working 時 reset 為 `0`，下次 `TryUnassignStaff` 設為 `now`（具 slot 能力職員從 Reallocating 進入 Working 時亦 reset 為 `0`，§3.5.3 step 8）|
...
| 情境 | 處理方式 |
|---|---|
| `TryAssignStaff` 指派建築 capacity 已滿 | return `BUILDING_FULL`；不變動 instance state |
| `TryAssignStaff` 同建築重複指派（idempotent，B-6 案）| return `SUCCESS`、不發 `OnStaffAssigned` 事件、不查冷卻、不重設 cooldown |
| `TryAssignStaff` 冷卻中（B-5 案）| return `SWITCH_COOLDOWN`；任何「進入 Working」transition 皆被擋 |
| `TryUnassignStaff` 對 OnLeave 職員 | return `STAFF_ON_LEAVE`；OnLeave 職員需先 `TryReturnFromLeave` |
| `TryFireStaff` 對 Working 職員（B-7 案）| 流程允許；**直接銷毀，不執行 unassign**；只發 OnStaffFired |
| `HireStaff` 後超 `REALLOCATING_AUTO_LEAVE_SECONDS` 未指派（具 slot 能力）| `CheckReallocatingAutoLeave` 自動轉 OnLeave（§3.7） |
| `HireStaff` 後超 `REALLOCATING_AUTO_LEAVE_SECONDS` 未指派（無 slot 能力）| **不轉假**，Reallocating 為穩態（§3.7.5） |
...
| StaffTable.salary 為負值 | DataManager 載入時拋 `StaffTableValidationException`（N-11 案：critical data fail-fast，與 §5.4 effects 處理一致）；salary = 0 為 Jam 版合法值（薪水未啟用） |
...
| `StaffTable.effectIDs` / `effectValues` 長度不一致 | DataManager 載入時拋 `StaffTableValidationException`（M-6 案：critical data fail-fast，與 §3.2 一致）|
...
| 解鎖瞬間（OnBuildingUpgraded buildingID=6 fromLevel=0 toLevel=1）| §3.10.5 訂閱 callback：`lastSalaryTimestamp ← now`（Phase 2 才生效）；同時為跨 `REALLOCATING_AUTO_LEAVE_SECONDS` 閾值的具 slot 能力職員 reset `reallocatingStartTimestamp ← now`（M-3 案，§3.7.5）|
| 降級後再解鎖 | StaffInstance[] 保留、effect 立即恢復查詢；Jam 版不重發歷史事件；reallocatingStart reset 行為見上一行 |
...
| 2 | F-02 Time System | `OnDailyReset`（Phase 2 薪水，M-2）、`OnMinuteTick` + 自節流每 3600s（自動轉休假檢查；F-02 無 OnHourTick） | 驅動 §3.7 / §3.9 |
| 3 | F-03 Resource Mgmt | `AddGoldAllowBankruptcy`（透過 FT-05 中介） | 解雇資遣費（§3.8.2）、薪水扣款（Phase 2 §3.9） |
| 4 | FT-07 Guild Building | `IsStaffSystemUnlocked()`、`GetBuildingState(buildingID).currentLevel` | 系統閘 + 各建築 capacity 取得 |
| 5 | FT-08 Gacha System | （單向被動接收）`HireStaff(candidate)` 由 FT-08 呼叫 | 接收錄用候選 |

> 註：FT-06 Guild Core `GetCurrentLevel()` **不在 FT-12 直接依賴清單**——`StaffTable.minGuildLevel` 由 FT-08 gacha 池於 candidate 過濾時使用（FT-08 §3.2），FT-12 入職流程僅驗證 staffID 合法性（§3.3.2）。
...
| 常數 | 預設值 | 用途 |
|---|---|---|
| `BUILDING_SWITCH_COOLDOWN_SECONDS` | `21600`（6h）| §3.6.4 進入 Working 觸發冷卻 |
| `REALLOCATING_AUTO_LEAVE_SECONDS` | `43200`（12h）| §3.7 自動轉休假觸發（具 slot 能力職員）|
...
| `[F-02]` | §6.2 下游列出 FT-12 為 `OnDailyReset`（Phase 2）/ `OnMinuteTick`（自節流，Jam 版訂閱） 訂閱者 |
...
**`RestoreFromSave(string ownerJson)` 行為**（M-5 案修訂）：

1. **反序列化** 上述 3 個欄位。
2. **staffID 合法性**：對每筆 `StaffInstance.staffID` 透過 `F-01.Get<StaffData>(staffID)` 驗證；找不到時拋 `CriticalRestoreFailedException`（FT-10 critical 路徑）。
3. **assignedBuildingID 合法性**：
   - `currentState == OnLeave` → 強制 assignedBuildingID = 0（不論存檔值，B-5 不變式）
   - `currentState == Reallocating` → 強制 assignedBuildingID = 0（B-5 不變式）
   - `currentState == Working`：
     - `assignedBuildingID == 0` 且**具 slot 能力**（StaffTable.slotBuildingIDs 含 > 0）→ **非法**：reset state = Reallocating、reallocatingStartTimestamp = now、`LogWarning`
     - `assignedBuildingID == 0` 且**無 slot 能力** → **非法**（無 slot 職員不該為 Working）：reset state = Reallocating、reallocatingStartTimestamp = 0（穩態）、`LogWarning`
     - `assignedBuildingID > 0` 且 buildingID 不合法（FT-07 查不到 / 該建築未建造） → reset assignedBuildingID = 0、state = Reallocating、reallocatingStartTimestamp = now、`LogWarning`
     - `assignedBuildingID > 0` 且 buildingID 不在 `StaffTable.slotBuildingIDs` → reset 同上、`LogWarning`
4. **Capacity 衝突解法**（M-5 新增）：對每個 buildingID 收集 Working 職員清單；若數量 > `FT07.BuildingTable[buildingID].slotCount`（如建築降級 / slotCount 改）：
   - 按 `hiredTimestamp` 升序保留前 N 位（N = capacity）
   - 其餘 reset assignedBuildingID = 0、state = Reallocating、reallocatingStartTimestamp = now、`LogWarning("staff {id} kicked from building {bid} due to capacity overflow")`
5. **`_nextInstanceID` 自動 max**（M-5 新增）：
   - `_nextInstanceID = max(_nextInstanceID, max(roster.Keys) + 1)`
   - 防止舊存檔欄位缺失或 instanceID 衝突造成新建 instance 重複 ID
6. **duplicate instanceID 檢查**：roster 反序列化時若同 instanceID 出現兩次，拋 `CriticalRestoreFailedException`（保留資料完整性、不靜默丟棄）
7. **state enum 合法性**：非 `Working` / `Reallocating` / `OnLeave` 三值 → 拋 `CriticalRestoreFailedException`
8. 還原完成**不補發任何事件**（UI 由 Bootstrap 後查詢刷新）
...
#### 7.1.2 `StaffTable.effectIDs` / `effectValues`（B-2 命名對齊 §3.2 enum）

每位職員攜帶 effect 清單（CSV 多值欄位以 `|` 分隔）；effectID 列舉名與 §3.2 `StaffEffect` enum 完全一致：

| effectID（enum） | 類型（§3.2） | effectValue 安全範圍 | 用途 |
|---|---|---|---|
| `Willingness` | Passive | `[0.0, 0.20]` | FT-03 NPC 接受意願加成（無需指派；Working / Reallocating 皆生效）|
| `AccountantCommission` | Passive（M-1）| `[0.0, 0.10]` | FT-05 委託傭金加成；**無需指派**——M-1 案改為 Passive，Working / Reallocating 皆生效 |
| `AccountantPenaltyOnVault` | Slot | `[-0.10, 0.0]` | FT-05 委託失敗罰款減免；需 Working AND assignedBuildingID == 5（預備金保險櫃）|
| `RecruitRefreshOnCounter` | Slot | `[0, 21600]`（秒）| FT-08 面試 auto refresh 間隔減免；需 Working AND assignedBuildingID == 4（公會櫃臺）|

聚合上限見 §4.1（疊加封頂規則）。

#### 7.1.3 `StaffTable.uiFlagIDs` / `uiFlagBuildingIDs`（UI flag 獨立路徑）

UI flag 走獨立聚合路徑（§3.4.6 OR 語意），**不**進 effectIDs / effectValues：

| uiFlagID（enum）| 對應 buildingID（uiFlagBuildingIDs[i]） | 對應 API | 消費者 |
|---|---|---|---|
...
| key | 預設值 | 安全範圍 | 影響 |
|---|---|---|---|
| `BUILDING_SWITCH_COOLDOWN_SECONDS` | `21600`（6h）| `[0, 86400]` | §3.6.4 進入 Working transition 觸發冷卻；過短會讓玩家頻繁切換 effect、過長挫折感 |
| `REALLOCATING_AUTO_LEAVE_SECONDS` | `43200`（12h）| `[3600, 86400]` | §3.7 自動轉休假觸發（具 slot 能力職員）；過短不友善（玩家剛雇就被轉假）、過長無效 |
...
- effect 數值校準從 `Willingness = 0.05`、`AccountantCommission = 0.02` 開始（B-2 命名對齊 §3.2 enum）；先驗證 §3.4 聚合上限不被輕易觸發
...
| AC-5 | 入職後新 instance state = `Reallocating`；具 slot 能力職員 `reallocatingStartTimestamp = now`、無 slot 能力職員（slotBuildingIDs 空或全 0）`reallocatingStartTimestamp = 0`（B-9 A）| 用 `GetInstance(id)` 驗證欄位 | EditMode test |

### 8.3 Slot 指派與三態狀態機

| AC | 條件 | 預期 | 驗證 |
|---|---|---|---|
| AC-6 | `TryAssignStaff(id, validBuildingID)`（state=Reallocating）| 回 `SUCCESS`、state→Working、發 `OnStaffAssigned + OnStaffStateChanged`、cooldown 觸發（B-5） | EditMode test |
| AC-7 | TryAssignStaff 同建築重複指派（idempotent，B-6） | 回 `SUCCESS`、不發事件、不查冷卻、cooldown 不重設 | EditMode test |
| AC-8 | TryAssignStaff 違反 capacity | 回 `BUILDING_FULL`、roster 不變 | EditMode test |
| AC-9 | TryUnassignStaff（state=Working）| state→Reallocating、發兩事件 | EditMode test |
| AC-10 | TryGoOnLeave（state=Working、oldBuildingID > 0）| 自動清 assignedBuildingID、state→OnLeave、發 `OnStaffAssigned(oldBuildingID, 0)` + `OnStaffStateChanged`（M-4 案）| EditMode test |
| AC-11 | TryReturnFromLeave（state=OnLeave）| state→Reallocating、reallocatingStart=now | EditMode test |
| AC-12 | Reallocating 超過 `REALLOCATING_AUTO_LEAVE_SECONDS` 且具 slot 能力 | OnMinuteTick + 自節流（每 3600s）觸發 `CheckReallocatingAutoLeave` 自動轉 OnLeave；無 slot 能力職員不轉假 | PlayMode test（時間快進）|
...
| AC-14 | 解雇 Working 職員（B-7 案）| **直接銷毀，不發 OnStaffAssigned / OnStaffStateChanged**；effect 立即停用（roster 移除後 §3.4 即時遍歷不再含此 instance） | EditMode test |
...
| AC-17 | 1 位 Working 或 Reallocating 會計（M-1 Passive，無需指派）| `GetAccountantCommissionBonus()` 回 `effectValue` | EditMode test |
...
> §3.9 / §4.2 / 本節 AC 標記 Phase 2，Jam 版整段跳過。Codex 工項書應將此節 AC（AC-21~AC-25 + AC-2）標為 `[Phase 2]`：

- **AC-2**（B-10 A 移入）：解鎖瞬間（OnBuildingUpgraded buildingID=6 0→1，§3.10.5 callback Phase 2 才訂閱）→ `_lastSalaryTimestamp` 設為 now
...
- **AC-36**（F-5 移入）：50 位職員 perStaffSalary 組裝 < 5ms（Profiler）
...
| AC-34 | reallocatingStartTimestamp == 0（舊存檔 / 無 slot 能力穩態）| CheckReallocatingAutoLeave 跳過；TryAssignStaff 進 Working 時 reset 為 0；TryUnassignStaff 設為 now | EditMode test |

### 8.10 效能與規模

| AC | 條件 | 預期 | 驗證 |
|---|---|---|---|
| AC-35 | 名冊 50 位職員時 effect 聚合 | 單次查詢 < 1ms（即時遍歷）；實作要求：禁用 LINQ、禁用每次配置 List/iterator、StaffData lookup 用 Dictionary O(1)；hot path 每 frame 可被多次呼叫 | Profiler |
