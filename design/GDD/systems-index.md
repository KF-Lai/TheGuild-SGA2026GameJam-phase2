...
_建立時間：2026-04-19_
_最後更新：2026-05-02_
_狀態：Phase 2 後端 18 系統實作完成（Foundation 3 + Core 6 + Feature 9，含 FT-02 A/B 拆分）；P-02 Main UI Framework 實作完成（FSD A/B 拆分共 25 Script）；FT-11 Jam 範疇外不實作；**P-03 Jam 範疇外，不實作**（2026-05-03 確認）；D-01 / D-02 GDD 已設計但 FSD 未啟動；**P-01 Jam 必做**（2026-05-03 校正，桌面透明視窗 widget 是視覺核心，A-01/A-14 art 設計皆以 widget 為前提；FSD 已通過 design-review 並轉「已完成」2026-05-03；Script 由 unity-specialist subagent fallback 落地 2026-05-03，DoD-01~DoD-12 待 Standalone build manual test 驗證）；**Jam 階段 NARRATIVE_ENABLED=0 停用劇情**（2026-05-03 確認，FT-09 / 奧蘿瑞女神 / 奧菲莉雅 disabled，runtime flag 控制）_
...
| ID   | 系統                           | 說明                       | GDD 狀態 | 對應資料表            |
| ---- | ------------------------------ | -------------------------- | -------- | --------------------- |
| D-01 | **Character Content Database** | 冒險者名字池、背景故事模板（單名；種族×職業 bio；gender/bio 存入 C-02 AdventurerInstance） | ✅ 已設計（2026-05-02） | `NamePool`, `BioPool` |
| D-02 | **Mission Content Database**   | 任務名稱池、描述模板       | ✅ 已設計 | `MissionNamePool`     |
...
| P-01 Desktop Transparent Window | ✅   | ✅                                       | ✅（2026-05-03 通過 design-review，使用者複核已轉「已完成」）| ✅（2026-05-03 subagent fallback 落地，6 Script + 1 SO + 1 asset，1308 行 + asmdef）| ⚠️（EditMode/PlayMode 既有 baseline 全綠；DoD-01~DoD-12 待 Standalone Win build manual test 驗證）|
...
| 2026-04-30 | **P3.1-aurorae**（含 P3.1-001 ~ P3.1-010）| **跨 10 個 GDD/FSD**：FT-09 / C-01 / FT-04 / C-05 / C-02 / FT-10 / FT-05 / FT-12 / FT-07 / P-02 | 各 GDD 多章節（詳見集中文件）| **奧蘿瑞女神陣營劇本（v3.1）所引發的 patch 候選清單**。涵蓋：(1) C-01 MissionTemplate 新增 `isScriptedDeath` / `minDangerLevel` / `requiredTraitID` 三欄位；(2) C-05 新增「沉默」trait（traitID=999, willingness_all -0.40，具名特殊角色突破安全範圍）+ `on_death_survive` 對 `isScriptedDeath=1` 無效規則；(3) FT-04 `RollSuccessAndDeath` 補 short-circuit + condition 過濾規則 + `styleTag jitter modifier`（FB-M1 偏負 4%）；(4) FT-09 大幅修改：`StoryStageTable` 新增 `dialogueVariantMode` / `specialEventKey` / `unlockBlockerCondition` 三欄位、`GetCurrentStyleTagBias() API`、`OnFactionStoryStageEpilogue` 新事件、`OnOpheliaMissingNight` / `OnOpheliaReturned` 事件、訂閱 FT-04 死亡事件累積 `_totalAdventurerDeaths`、`_blockedStages` 機制；(5) C-02 新增 `RegisterUniqueAdventurer` API + `DismissAdventurer` 規則放寬（審查處解鎖後 Idle 可開除）+ 名冊排序奧菲莉雅永遠第一格；(6) FT-10 `InitializeAsNewGame` 補奧菲莉雅初始化步驟 + 持久化 v3.1 新欄位；(7) FT-05 `SelectMissionFromPool` 補 `minDangerLevel` 篩選（先 weights 後過濾，最多 3 次 fallback）；(8) FT-12 新增 `IsStaffHired` API + `StaffTable` 5 個 Post-Jam 預埋欄位（`personalityDesc` / `intimacyLevel` / `isLeavePossible` / `mood` / `personalEventIDs`）+ 三位敘事核心職員（米拉 501 / 譚恩 502 / 凱拉 503）CSV 行；(9) FT-07 確認審查處解鎖機制；(10) P-02 `SceneObjectController` + `SceneObjectStateTable.csv` 設計需求登記（已於 2026-05-01 五輪 design-review APPROVED）。**完整 patch 內容見** `design/_Reports/GDD-FSD-patch-v3.1-aurorae-faction.md` | narrative-director 與 game-designer 7 次討論（2026-04-30）|

| 2026-04-30 | **P3.1-aurorae-design-review** | 跨 4 GDD：C-05 / C-02 / FT-04 / FT-09 | 補強章節 | **design-review 結果（4 項 P0 全 APPROVED）+ 補強紀錄**：(1) **C-05 P3.1-002**：APPROVED，無 follow-up；(2) **C-02 P3.1-005 + R2**：APPROVED with concerns，§5 Idle 除名 LogWarning 已存在於 §5.2 line 286（驗證通過）；(3) **FT-04 P3.1-003**：APPROVED with concerns，§6.1 上游依賴表已補新增「FT-04 ⇄ FT-09 雙向依賴」row（query + event 雙向，runtime 順序無循環，實作以 Service Locator / DI 解耦）；(4) **FT-09 P3.1-004 + R1+R3**：APPROVED with concerns，§5.1 EC 索引表已補 **EC-13** 奧菲莉雅被解雇後 Stage 5 永久 blocked（設計自洽接受，敘事上「沒有奧菲莉雅，沒有 Stage 5」）。**整體判定**：v3.1 patch 全部 APPROVED，無 blocker。修正建議都是文件補強（明示既有設計意圖），不需要實質修改 patch 內容。**待後續**：(a) FT-07 IsBuildingUnlocked API verify（已併入 Stage E 腳本確認）；(b) C# Script 實作（依 patch summary §1 P0/P1/P2 順序，待 Codex / gameplay-programmer 介入）。 | Claude Code 主體（Opus 4.7 + xhigh）design-review |
| 2026-05-02 | **P-D01-001** | 跨 2 GDD：C-02 / FT-10（外加 D-01 設計新增） | C-02 §3.1 / §3.2 / §4.3 / §4.3a / §6.1 / §6.4 / §九；FT-10 §3.3.3 row 3 / §6.1 row 4 | **D-01 Character Content Database 設計確定後的跨系統 patch**：(1) **C-02 v1.1**：`AdventurerInstance` 新增 `gender:int` / `bio:string` 欄位（§3.1）；`AdventurerTemplate` 新增 `bio:string` 欄位（§3.2 / DS）；`CreateFromTemplate` 偽碼補 gender=0 / bio 複製（§4.3）；`CreateRandomInstance` 偽碼改用 `D01.PickRandomNameWithGender` + `D01.GetRandomBio`（§4.3a）；§6.1 上游依賴新增 D-01 列；§6.4 ISaveable 序列化欄位列表新增 gender / bio。(2) **FT-10**：§3.3.3 拓撲順序 row 3（C-02 Adventurer）標明含 gender / bio；§6.1 上游消費表 row 4（C-02 Adventurer Mgmt）標明 ISaveable 含 gender / bio。(3) **D-01 v1.0**：新建 GDD（單名設計、種族×職業 bio、bio 一次性生成存入 C-02）。 | `/design-system D-01` 設計成果 |
| 2026-05-01 | **P-02-design-review-v0.1~v0.6** | 跨 5 GDD：P-02 / FT-12 / FT-08 / P-01 / P-03 / FT-09 | 各 GDD 多章節（詳細見對應 §九 變更歷史）| **P-02 Main UI Framework 五輪 design-review APPROVED 後跨系統補強紀錄**：(1) **P-02 v0.3 → v0.6**：8 節全寫；二輪修正 9 類 cross-system API mismatch（HireSelected → TryRecruit / GetActiveCommissions → GetCommissionsBySource / GetCurrentLevelName → GetDangerData / GetMaxUnlockedStage → GetUnlockedStageIndex 等）；三輪修正對話隊列模型（_pendingDialogueCount → _pendingStageDialogueQueue FIFO）+ 6 處 stale 文件引用；四輪修正 BindLogWindow caller direction 矛盾 + Bootstrap 時序雙重保險；五輪 APPROVED；(2) **FT-12 §3.6.7（Pkg-5）**：補 `GetActiveRoster() : IReadOnlyList<StaffInstance>` + `GetRosterCap() : int` 公開查詢 API；(3) **FT-08 §3.3.4（Pkg-6）**：補 `GetCurrentCandidates()` + `GetCurrentPoolID()` query API（FT-08 不發 gacha 業務事件，P-02 採 polling 模式）；(4) **P-01 §3.8（Pkg-3）**：補對外 API surface 8 個方法（GetEffectiveScale / SetUserScale / ResetUserScaleToDefault / GetUserScale / EnumerateAvailableMonitors / SwitchTargetScreen / GetCurrentTargetMonitorID / Minimize）+ `RegisterEffectiveScaleListener` callback 註冊機制 + `MonitorInfo` schema；(5) **P-03 §3.5（Pkg-4）**：補 `BindLogWindow(VisualElement container) : BindResult` API + 三種回傳碼（Success / AlreadyBound / InvalidContainer）+ 雙端 contract 明示（P-02 為 caller，P-03 為 callee）；(6) **FT-09 §3.7.1**：補 `GetPendingDialogueStages() : IReadOnlyList<int>` API（P-02 §3.8 step 3 bootstrap 復原專用，與 FT-09 Step F 重發機制構成雙重保險）。**整體判定**：P-02 GDD APPROVED；無 blocker；剩餘 ⏳ 雙向依賴補強 5 條（FT-07 / C-02 / FT-04 / F-03 範圍）依 DIP-index 流程批次處理；P-03 §3.5 step 1 stale step 7 → step 8 已隨手修正 | Claude Code 主體（Opus 4.7 + xhigh）design-review |
