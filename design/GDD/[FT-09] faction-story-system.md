...
| 5 | `[FT-02] mission-dispatch.md` | 新增 §3.9 章節:`InjectStaticMission(missionID) → InjectStaticMissionResult` API + `_staticMissionPool: List<int>` + `_regularMissionPool: List<int>` 雙池結構（含 SaveData 持久化）+ **`OnCommissionPosted(int missionID, CommissionSource source)` 事件**（統一 Regular/Static 雙來源,取代設計初期的 `OnStaticMissionPosted`）+ 派遣後從池移除規則;§6 反向依賴登記 FT-09 為呼叫方 | ✅ 已寫入（2026-04-26 design-review C2/C5 修補;FT-02 §3.9 已補章節） |
...
| 9 | `[systems-index.md]` | 新增 `MissionFactionScoreWeight` 資料表登記；同步 `StoryStageTable` 命名（原 `StoryNodeTable`）；確認 FT-09 在系統索引內列為 Day 7-8 系統 | ✅ 已寫入(2026-04-26 design-review 修補) |
...
| `_unlockedStageIndices` | `Dictionary<int,int>` | `List<UnlockedStageEntry>` | 各陣營已解鎖的最新 stageIndex（factionID → stageIndex） |
