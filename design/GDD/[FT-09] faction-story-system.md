...
- 依賴 GDD 約束：
  ...
  - `[C-06] world-danger-system.md` — `OnFactionScoreUpdated(int newMaxScore)` 主動推送契約（C-06 §3.3 / §4.3.5 / §6.3）
  - `[F-01] data-manager.md` — `Get<FactionRouteData>` / `Get<StoryStageData>` / `GetWhere` 查詢（F-01 §6 line 185）
  - `[FT-08] guild-staff-system.md` — Schema 層預留（`StaffTable.factionID` 與 `StaffGachaPoolTable` 5 個預留閘）；Jam 版**無 runtime 依賴**
...
**FT-09 Faction Story System 是公會的線性陣營劇情系統**：透過訂閱 FT-04 `OnMissionResolved` 事件，依任務 `factionID` 與任務難度從 `MissionFactionScoreWeight` 取加權分數（僅成功結算計入），更新陣營累積分數；當該分數越過 `StoryStageTable.triggerCondition.threshold` 時解鎖下一劇情階段，以「對話視窗 + 特殊靜態委託」雙軌呈現——FT-09 發布 `OnFactionStoryStageUnlocked` 事件供 P-02 開啟對話視窗（讀 `dialogueKey`），玩家確認後 FT-09 透過 FT-02 將階段對應的 `missionID`（`MissionTemplate.categoryID = 3`）注入委託板供玩家手動接單。系統涵蓋三大職責：

1. **分數累積（Score Accrual）**：訂閱 FT-04 `OnMissionResolved`，依 `Outcome.missionFactionID` / `Outcome.isSuccess` / `Outcome.missionDifficulty` 從 `MissionFactionScoreWeight` 取加權分數，更新對應陣營累積值；`factionID == FACTION_NEUTRAL_ID(0)` 或 `isSuccess == false` 一律不計入
...
- 跨陣營對抗（成功一邊扣另一邊）
- FT-08 職員陣營傾向影響分數（FT-08 已預留 `StaffTable.factionID`，FT-09 Jam 版不消費）
- 多分支劇情樹（階段線性，不支援分歧）
...
| `MissionFactionScoreWeight` 載入成功且涵蓋全部 9 種難度（F~SSS） | 必要 |
...
Bootstrap(SaveData? saveData):
    Step A: DataManager.Get<FactionRouteData>() / Get<StoryStageData>() / Get<MissionFactionScoreWeightData>()
...
本節定義 FT-09 的 3 張 CSV 表：`FactionRouteTable`（§3.2.1）、`StoryStageTable`（§3.2.2）、`MissionFactionScoreWeight`（§3.2.3，新表）。引用既有常數 `FACTION_NEUTRAL_ID = 0`（`SystemConstants.csv`），FT-09 不新增 SystemConstants 條目。

#### 3.2.1 FactionRouteTable

**檔案位置**：`Assets/Resources/Data/FactionRouteTable.csv`
...
**檔案位置**：`Assets/Resources/Data/StoryStageTable.csv`
...
#### 3.2.3 MissionFactionScoreWeight（新表）

**檔案位置**：`Assets/Resources/Data/MissionFactionScoreWeight.csv`

**Schema**：

| 欄位 | 型別 | 說明 |
|---|---|---|
| `difficulty` | string (PK) | 任務難度；**必須涵蓋全部 9 種**（F / E / D / C / B / A / S / SS / SSS） |
| `factionScoreDelta` | int | 該難度任務「成功結算且 `factionID != 0`」時對應陣營的加分數值（**必須 ≥ 0**） |

**驗證規則**（DataManager 載入時）：

- 9 種難度任一缺失 → `MissionFactionScoreWeightTableValidationException("missing difficulty: {d}")`，整張表載入失敗 → FT-09 進入降級（§3.1.1）
- `factionScoreDelta < 0` → `Debug.LogError("difficulty={d}: negative weight not allowed")`，該難度視為 `0`

**Jam 預設**（呼應原設計「dark 路線靠高難度任務累積」精神）：
...
> 設計意圖：權重按難度近似指數遞增，讓 S+ 高難度任務在陣營推進中具備壓倒性貢獻——對應原設計「dark 路線主要透過 S 級任務累積」的精神，同時讓中低難度也能緩慢推進，避免新手玩家完全無進度體感。安全範圍與校準注意事項見 §7。
...
HandleOnMissionResolved(Outcome outcome):
    ...
    Step 2: 計算加分數值
        delta = MissionFactionScoreWeight[outcome.missionDifficulty].factionScoreDelta
        IF delta == 0:
            return  // 該難度權重為 0，不變動
...
```csharp
public readonly record struct OnFactionStoryStageUnlocked(
    int    stageID,
    int    factionID,
    int    stageIndex,
    int    missionID,
...
| **P-03 Notification System**（可選） | 推播桌面通知「新劇情解鎖」 |
...
2. 維護一個獨立的 `_staticMissionPool: List<int>`（runtime；持久化於 SaveData）
3. 將 `missionID` 加入 `_staticMissionPool`，並發布 `OnStaticMissionPosted(missionID)` 給 P-02 訂閱以更新委託板 UI
4. 派遣（`Dispatch`）成功後，**從 `_staticMissionPool` 移除該 `missionID`**（不重置回常規池）
...
T5  FT-02 將 missionID 加入 _staticMissionPool、發布 OnStaticMissionPosted
T6  P-02 委託板 UI 重新整理 → 玩家可見劇情委託
...
| `OnStaticMissionPosted` | 委託板 UI 顯示劇情委託 | FT-02 §（新章節） |
...
HandleOnMissionResolved(Outcome outcome) [在 §3.3.2 Step 6 後追加]:
    ...
    Step 10: 路線完結判定（§3.6.3）
        CheckRouteCompletion(stage.factionID)
```
...
```csharp
public readonly record struct OnFactionStoryStageResolved(
    int  stageID,
    int  factionID,
    int  stageIndex,
    int  missionID,
...
| **P-03 Notification System**（可選） | 推播桌面通知「劇情委託結算」 |
...
```
CheckRouteCompletion(int factionID):
    stages = StoryStageTable.GetByFactionID(factionID)
    ...
    EventBus.Publish(new OnFactionRouteCompleted(
        ...
        finalIsSuccess:  /* 從 outcome 取，見下方 */,
        totalStages:     maxIndex
    ))
```

> 由於 `CheckRouteCompletion` 在 §3.6.2 Step 10 呼叫，呼叫時 `outcome` 仍在範疇——`finalIsSuccess` 由該次 `outcome.isSuccess` 帶入。
...
```csharp
public readonly record struct OnFactionRouteCompleted(
    int  factionID,
    int  finalStageID,
    bool finalIsSuccess,
    int  totalStages
...
| **P-03 Notification System**（可選） | 推播桌面通知「陣營路線完結」 |
...
- 階段 N 解鎖後，玩家派遣階段 N 的劇情委託成功 → 該陣營 `currentScore` 額外 +`MissionFactionScoreWeight[difficulty].factionScoreDelta`
...
| `OnFactionStoryStageUnlocked` | §3.4.2 Step 3 / §3.1.2 Step F | `(int stageID, int factionID, int stageIndex, int missionID, string dialogueKey)` | P-02、P-03（可選） |
| `OnFactionStoryDialogueConfirmed` | §3.5.2 Step 6 | `(int stageID, int factionID, int missionID)` | P-02、P-03（可選） |
| `OnFactionStoryStageResolved` | §3.6.2 Step 9 | `(int stageID, int factionID, int stageIndex, int missionID, bool isSuccess, bool isDead)` | P-02、P-03（可選） |
| `OnFactionRouteCompleted` | §3.6.3 | `(int factionID, int finalStageID, bool finalIsSuccess, int totalStages)` | P-02、P-03（可選）、C-06（Post-Jam） |
...
{
    public Dictionary<int, int> factionScores;          // _factionScores
    public Dictionary<int, int> unlockedStageIndices;    // _unlockedStageIndices
...
> `Queue<int>` 與 `HashSet<int>` 序列化時轉為 `List<int>`，反序列化時重建——Unity JsonUtility / 第三方序列化器多數不直接支援這兩種容器。
...
| 取難度權重 | `DataManager.Get<MissionFactionScoreWeightData>().GetByDifficulty(difficulty)` | F-01 §6 / §3.2.3 |
...
> 本章彙整 FT-09 的 5 條核心公式。F-1 ~ F-3 為 runtime 執行公式；F-4 / F-5 為設計師於資料表校準時使用的工具公式（不在 runtime 執行，但決定 §3.2.2 / §3.2.3 數值合理性）。

### 4.1 F-1：陣營分數累積（Score Accrual）
...
```
delta(difficulty) =
    MissionFactionScoreWeight[difficulty].factionScoreDelta
...
| `delta(difficulty)` | int | 該難度任務加分權重 | `MissionFactionScoreWeight.csv`（§3.2.3） |
...
### 4.2 F-2：陣營最高分（Max Faction Score）
...
### 4.3 F-3：階段解鎖判定（Stage Unlock Check）
...
| `delta(difficulty)` | int | 該難度的權重 | `MissionFactionScoreWeight.csv`（§3.2.3） |
...
### 4.5 F-5：解鎖節奏推估（playtest 校準工具）

> **執行模型**：本公式**不在 runtime 執行**，是設計師於 playtest 後對比實際數據、校準 `MissionFactionScoreWeight` 或 `StoryStageTable.scoreThreshold` 的工具；亦支援 §7 Tuning Knobs 的安全範圍預估。
...
> 本章採**混合格式**：EC-1 ~ EC-9 為 §3 已內嵌規範的場景，採「觸發 + 行為摘要 + 引用源」短條目；EC-10 ~ EC-12 為 §3 未明確規範或屬於跨系統時序的場景，採「觸發 / 行為 / 玩家可見 / 規範細節」完整展開。每條 EC 皆遵守 `design-docs.md`「explicitly state what happens」原則——不留「應優雅處理」這類空話。

### 5.1 EC-1：CSV 載入失敗或部分缺失

| 項目 | 內容 |
|---|---|
| **觸發** | (a) `FactionRouteTable` 全為 neutral / 空表；(b) `StoryStageTable` 空表；(c) `MissionFactionScoreWeight` 缺任一難度（F~SSS 9 種）；(d) 三表任一檔案不存在 |
...
### 5.2 EC-2：runtime 收到未知 factionID 的 OnMissionResolved
...
### 5.7 EC-7：FT-02.InjectStaticMission 注入失敗
...
### 5.8 EC-8：P-02 順序錯誤呼叫 ConfirmDialogue
...
### 5.9 EC-9：劇情委託本身計分跨越下一階段 threshold

| 項目 | 內容 |
|---|---|
| **觸發** | 階段 N 解鎖、玩家派遣階段 N 劇情委託成功、其加分 (`MissionFactionScoreWeight[difficulty]`) 直接使 `newScore >= scoreThreshold(N+1)` |
...
### 5.10 EC-10：SaveData 反序列化異常 / 欄位缺失
...
### 5.12 EC-12：FT-09 Bootstrap 與依賴系統的時序保證
...
| EC | 場景 | §3 覆蓋 | §5 處理方式 |
|----|------|---------|-----------|
| EC-1 | CSV 載入失敗 | §3.1.1 / §3.1.3 / §3.2 | 引用摘要 |
| EC-2 | 未知 factionID | §3.3.2 / §3.3.3 | 引用摘要 |
| EC-3 | 玩家未確認對話關遊戲 | §3.1.2 / §3.5.4 | 引用摘要 |
| EC-4 | 同 frame 多階段解鎖 | §3.4.3 | 引用摘要 |
| EC-5 | 劇情委託失敗 / 死亡 | §3.6.1 / §3.6.2 | 引用摘要 |
| EC-6 | 玩家忽略所有劇情委託 | §3.6.6 | 引用摘要 |
| EC-7 | InjectStaticMission 失敗 | §3.5.2 / §3.5.4 | 引用摘要 |
| EC-8 | P-02 順序錯誤 | §3.5.2 / §3.5.4 | 引用摘要 |
| EC-9 | 劇情委託計分跨越下一階段 | §3.6.7 | 引用摘要 |
| EC-10 | SaveData 反序列化異常 | 未涵蓋 | 完整展開 |
| EC-11 | 路線完結後計分 / 事件 | 未涵蓋 | 完整展開 |
| EC-12 | Bootstrap 時序失準 | 部分（§3.1.2 假設） | 完整展開 |
...
| 項目 | 內容 |
|---|---|
| **接觸點** | 呼叫 API `DataManager.Get<FactionRouteData>()`、`Get<StoryStageData>()`、`Get<MissionFactionScoreWeightData>()` |
| **本端用途** | Bootstrap §3.1.2 Step A 一次性載入；runtime 各處讀表（§3.7.5 查詢路徑表） |
| **對端契約** | F-01 §6 line 185 須登記 3 個 Data 型別的查詢支援；CSV 載入失敗時回 null（FT-09 自行處理為降級） |
...
| 項目 | 內容 |
|---|---|
| **接觸點** | 呼叫 API `C-01.GetTemplate(missionID)` 讀 `MissionTemplate.categoryID` 與 `factionID` |
| **本端用途** | §3.6.2 Step 7 識別劇情委託（`categoryID == 3`） |
| **對端契約** | C-01 §3.2 須維持 `MissionTemplate.categoryID` / `factionID` 欄位；C-01 §6 line 189 已登記 categoryID 語意（3 = faction_story） |
| **失準行為** | `GetTemplate` 回 null → §3.6.2 Step 7 視為非劇情委託，跳過；不阻擋常規結算流程 |

#### 6.1.4 FT-02 Mission Dispatch（新 API）

| 項目 | 內容 |
|---|---|
| **接觸點** | 呼叫 API `FT-02.InjectStaticMission(int missionID) → InjectStaticMissionResult` |
| **本端用途** | §3.5.2 Step 4 將劇情委託 `missionID` 注入委託板靜態池 |
| **對端契約** | FT-02 須**新增 §3.x 章節** 定義：`InjectStaticMission` API + `_staticMissionPool` runtime 結構 + `OnStaticMissionPosted` 事件 + 派遣後從池移除規則（詳見 §3.5.3） |
...
| **對端職責** | C-06 須提供 `OnFactionScoreUpdated` public method，接收後更新 `_cachedMaxFactionScore` 並驅動陣營閘判定（C-06 §3.3 / §4.3.5） |
| **對端文件** | C-06 §3.3 / §4.3.5 / §6.3 已定義契約，§6 須登記 FT-09 為推送方 |
...
#### 6.2.4 FT-08 Guild Staff System（Jam 版無 runtime 依賴）

| 項目 | 內容 |
|---|---|
| **接觸點** | **Jam 版無** runtime 事件 / API 連接 |
| **Schema 預留** | FT-08 `StaffTable.factionID` 與 `StaffGachaPoolTable` 的 5 個預留閘（factionID / minDifficulty 等）為 **Post-Jam 啟用** |
| **對端文件** | FT-08 GDD 須在「設計來源」章節聲明「Jam 版**無 FT-09 runtime 依賴**；schema 預留 staff factionID 欄位，Post-Jam 才啟用 FT-09 → FT-08 的職員陣營傾向消費」（FT-09 §1「不在 Jam 範疇」第 3 條已對映） |
| **未來契約**（Post-Jam 提示） | Post-Jam 若 FT-08 消費陣營分數，建議走 `OnFactionScoreChanged` 訂閱模式，避免 FT-09 主動呼叫 FT-08（保持 FT-08 為下游） |
...
| 2 | F-01 | FT-09 → F-01 | 呼叫 `Get<FactionRouteData / StoryStageData / MissionFactionScoreWeightData>()` | §3.1.2 / §3.7.5 | 3 個 Data 型別查詢 + 載入失敗回 null | F-01 §6 line 185 |
| 3 | C-01 | FT-09 → C-01 | 呼叫 `GetTemplate(missionID)` 讀 `categoryID` / `factionID` | §3.6.2 Step 7 | `MissionTemplate.categoryID` 含 `3 = faction_story` 語意 | C-01 §3.2 / §6 line 189 |
| 4 | FT-02 | FT-09 → FT-02 | 呼叫 `InjectStaticMission(missionID)` | §3.5.2 / §3.5.3 | **新章節**：`InjectStaticMission` API + `_staticMissionPool` + `OnStaticMissionPosted` 事件 | FT-02 §3.x（**待補**） |
...
| 6 | C-06 | FT-09 → C-06 | 呼叫 `OnFactionScoreUpdated(maxScore)` | §3.1.2 / §3.3.5 | public method + `_cachedMaxFactionScore` + 陣營閘判定 | C-06 §3.3 / §4.3.5 / §6.3 |
...
| 9 | FT-08 | 無 runtime 連接 | （Jam 版無） | §1 | Schema 預留聲明 + Post-Jam 啟用提示 | FT-08「設計來源」章節 |

> **檢查方法**：本表 9 條依賴 → 對端文件須在其「設計來源」/「§6 依賴」章節登記 FT-09 為對端方（含本表 # 6 ~ # 9 的訂閱方向）；缺漏項目於 §6.4 列出。

---

### 6.4 反向依賴更新清單（待對端 GDD 補登記）

> 由於 FT-09 為新系統 + 部分上游 / 下游 GDD 仍處於草案 / 待設計階段，以下 9 個 GDD 須在 FT-09 通過審查後補登記反向依賴。本表作為後續批次更新的工項清單（與 session state 內清單同步）。

| # | GDD | 待更新內容 | 行動類型 |
|---|------|----------|---------|
| 1 | `[FT-04] outcome-resolution.md` | §6 反向依賴登記 FT-09 為 `OnMissionResolved` 訂閱者；確認事件 payload 含 `isDead` 欄位 | **登記** |
| 2 | `[C-01] mission-database.md` | §6 反向依賴登記 FT-09 為 `categoryID = 3` 消費者；同步 `StoryStageTable` 命名（原 `StoryNodeTable`） | **登記** + **改名** |
| 3 | `[C-06] world-danger-system.md` | §6 反向依賴登記 FT-09 為 `OnFactionScoreUpdated` 推送方；確認 `_cachedMaxFactionScore` 接收契約 | **登記** |
| 4 | `[F-01] data-manager.md` | §6 line 185 登記 `Get<FactionRouteData>` / `Get<StoryStageData>` / `Get<MissionFactionScoreWeightData>` 三個查詢路徑 | **登記** |
| 5 | `[FT-02] mission-dispatch.md` | **新增 §3.x 章節**：`InjectStaticMission(missionID) → InjectStaticMissionResult` API + `_staticMissionPool: List<int>`（含 SaveData 持久化）+ `OnStaticMissionPosted(missionID)` 事件 + 派遣後從池移除規則；§6 反向依賴登記 FT-09 為呼叫方 | **新章節** + **登記** |
| 6 | `[FT-08] guild-staff-system.md` | 「設計來源」章節聲明：「Jam 版**無 FT-09 runtime 依賴**；FT-08 schema 預留 `StaffTable.factionID` 與 `StaffGachaPoolTable` 5 個預留閘，為 Post-Jam 啟用 FT-09 → FT-08 的職員陣營傾向消費而保留」 | **聲明** |
| 7 | `[P-02] main-ui-framework.md`（待設計） | 列為 P-02 設計輸入：對話視窗管理（FIFO + 隊首匹配）+ 委託板 UI（categoryID = 3 視覺差異）+ 結算面板（StageResolved epilogue）+ 路線完結 UI；訂閱 5 個 FT-09 事件 + 呼叫 `ConfirmDialogue` / `FT-02.Dispatch` | **設計輸入** |
| 8 | `[P-03] notification-system.md`（待設計） | 列為 P-03 設計輸入：4 個 FT-09 事件可選訂閱（StageUnlocked / DialogueConfirmed / StageResolved / RouteCompleted） | **設計輸入** |
| 9 | `[systems-index.md]` | 新增 `MissionFactionScoreWeight` 資料表登記；同步 `StoryStageTable` 命名（原 `StoryNodeTable`）；確認 FT-09 在系統索引內列為 Day 7-8 系統 | **新表** + **改名** |

**處理時機**：依使用者於 `project_ft05_reverse_deps` memory 確立的策略——FT-09 整體（§1 ~ §8）通過審查後**一次批次處理**這 9 個 GDD 更新；§6 寫入時**不立即修改下游 GDD**。

**驗證方式**：上述 9 個 GDD 全部完成後，於 §6.3 檢查表逐條 grep 對端文件是否登記 FT-09 / FactionStory 關鍵字；缺漏者重新檢視。
...
|------|------|------|----------------|
| **A. 資料表旋鈕（CSV）** | 5 | `Assets/Resources/Data/*.csv` | ✅ 直接影響 |
...
| ID | 旋鈕 | 類型 | 影響面 |
|----|------|------|-------|
| A1 | `MissionFactionScoreWeight.factionScoreDelta`（9 個值，每難度一個） | A | 全域分數累積速度 |
...
#### 7.2.1 A1：MissionFactionScoreWeight.factionScoreDelta

| 欄位 | 內容 |
|---|---|
| **位置** | `Assets/Resources/Data/MissionFactionScoreWeight.csv`，9 行（F~SSS 每難度一行） |
| **預設值（Jam）** | F=1, E=1, D=2, C=3, B=5, A=8, S=13, SS=20, SSS=30（§3.2.3） |
...
| 欄位 | 內容 |
|---|---|
| **位置** | `Assets/Resources/Data/StoryStageTable.csv`，每階段一行的 `scoreThreshold` 欄 |
| **預設值（Jam 秩序）** | `[10, 30, 60]` |
...
| 欄位 | 內容 |
|---|---|
| **位置** | `Assets/Resources/Data/FactionRouteTable.csv`，有效行數（`factionID != 0`） |
...
| 欄位 | 內容 |
|---|---|
| **前置** | (a) Valid CSV：3 表齊全 + 至少 1 條 valid 路線；(b) Invalid CSV：FactionRouteTable 全 neutral；(c) 缺失：MissionFactionScoreWeight 缺 SS 行 |
...
| AC-D-9 | FT-08 | Jam 版**無 runtime 連接** | grep FT-09 程式碼不應引用任何 FT-08 識別符號；FT-09 與 FT-08 各自獨立啟用 |
...
| FT-08 職員陣營傾向影響分數 | §1 + §6.2.4 + AC-D-9 |
| 多分支劇情樹（階段分歧） | §1「階段線性」 |
