...
| `triggeredConditionTraits` | `int[]` | 實際觸發（機率判定通過）的 condition traitID；供 P-03 通知與 Debug UI。`BuildOutcomeSnapshot` 初始化為 `new int[0]`（空陣列），由 C-05 § 4.4 `ApplyConditionTraits` 在機率判定通過時追加 |
...
ApplyAdventurerStatus(outcome):
    switch outcome.finalStatus:
        case Idle:    C02.UpdateStatus(outcome.adventurerInstanceID, Idle)    // 走 default missionInstanceID = 0 overload
        case Wounded: C02.SetWounded(outcome.adventurerInstanceID)
        case Dead:    C02.UpdateStatus(outcome.adventurerInstanceID, Dead)    // 走 default missionInstanceID = 0 overload
```

- `UpdateStatus(id, status)` 兩參數呼叫對應 C-02 §3.5 既定簽名 `UpdateStatus(int instanceID, AdventurerStatus newStatus, int missionInstanceID = 0)` 的 default overload；missionInstanceID 省略時為 `0`，C-02 內部依 newStatus 維護 `currentMissionID` invariant
- `C02.UpdateStatus(id, Idle)` 對 `Dispatched` 狀態呼叫時自動清除 `currentMissionID`、設 `idleSinceTimestamp = NowUTC`（由 C-02 § 3.5 合約保證）
- `C02.SetWounded` 依 C-02 § 4.1 規格計算並寫入 `woundedUntilTimestamp`（`currentUTC + WOUNDED_RECOVERY_HOURS × 3600`），並自動清除 `currentMissionID`
- `C02.UpdateStatus(id, Dead)` 自動清除 `currentMissionID = 0`、`idleSinceTimestamp = 0`，冒險者保留於名冊直至手動除名
...
| **P-03 Notification** | 依 `finalStatus`、`triggeredConditionTraits` 選擇通知模板，推播桌面通知 **【→Log API待更新】** |
...
| `baseDelta` | `+8` | B 難度 `successDelta`（§3.6 表格） |
| `conditionDelta` | `0` | — |
...
| **P-03 Notification System** | Event: `OnMissionResolved(Outcome)` | 依 `finalStatus` + `triggeredConditionTraits` 選擇 `NotificationTemplate`，推播桌面通知 **【→Log API待更新】** | `finalStatus`, `triggeredConditionTraits`, `missionDifficulty` |
...
| 欄位 | 型別 | 預設值範圍 | 安全範圍 | 影響 |
|------|------|----------|---------|------|
| `successDelta` | `int` | `+3 ~ +12`（見 § 3.6） | `[+1, +30]`（若超過，會讓聲望上升過快，破壞 GuildLevelTable 門檻節奏） | 成功任務的聲望回報。建議維持「難度愈高 delta 愈大」的單調遞增，並確保 F ~ SSS 總跨距約為 4 倍（目前 +3 : +12） |
| `failDelta` | `int` | `-1 ~ -15`（見 § 3.6） | `[-30, -1]`（若超過 `-30`，單次失敗易觸發破產警告，過度懲罰離線結算） | 失敗任務的聲望扣分。建議保持「失敗懲罰略高於成功回報」（高難度尤其明顯，SSS 為 `+12 / -15`），鼓勵穩健決策 |

> **設計意圖註記（A 階 successDelta 反直覺低於 B 階）**：§3.6 表格中 `A successDelta = +6 < B successDelta = +8`，並非 typo。理由：A 階任務對應「高 maxRecruitableRank 公會的常規任務」，公會等級高的玩家已能穩定接 A，若給予爆量聲望會破壞中後期節奏；B 階對中等公會反而是「踮腳挑戰」，聲望回報略高鼓勵跨階接單。failDelta 仍維持單調遞增（`-6 < -8`），因高難度失敗的代價應與難度成正比。若 playtest 證實此節奏失衡，可調整為 A successDelta = +9~+10。
...
| ID | 驗收條件 | 驗證方式 |
|----|---------|---------|
| AC-OR-21 | 成功 B 難度任務（無 condition）結算後，F-03 `GetReputation()` 相較結算前增加 `+8`（ReputationDeltaTable[B].successDelta） | Unit test：結算前後比對 |
