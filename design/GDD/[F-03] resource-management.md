...
Resource Management 是遊戲中金幣與聲望兩項核心資源的唯一管理者。所有系統若需增減資源，必須透過 Resource Management 的 API（`AddGold`、`AddReputation`）執行，不得直接修改資源值。金幣硬性下限由 C-06 World Danger System 的 `GetMaxDebt()` 決定（危險度越高，允許負債越深），上限為 9,999,999；聲望限制在 -100 至 100 之間。金幣進入負值時啟動破產倒數警告，倒數期長度由 `BankruptcyThresholdTable` 依聲望段查詢。Resource Management 同時負責在資源變動時廣播事件，供 UI 與其他系統即時更新。
...
| 資源 | 型別 | 初始值 | 範圍 |
|------|------|--------|------|
| 金幣（Gold） | `int` | `100` | `C-06.GetMaxDebt()` ~ `GOLD_MAX (9,999,999)` |
...
5. 金幣硬性下限（`GetCurrentBankruptcyThreshold()`）由 C-06 `GetMaxDebt()` 決定；F-03 訂閱 `C-06 OnDangerLevelChanged` 事件更新快取值 `_currentMaxDebt`；危險度只升不降，下限只會隨遊戲進行變得更寬鬆（更負）
6. **初始同步**：F-03 在 `Start` 階段呼叫 `C-06.GetMaxDebt()` 取得 `_currentMaxDebt` 初始值（C-06 在 `Awake` 階段已將初始危險度 E 的 `maxDebt = -100` 設定完成）
...
7. **初始化順序**：F-03 的 Script Execution Order 必須晚於 F-01 DataManager、F-02 Time System 與 C-06 World Danger System。`Awake` 階段從 DataManager 讀取常數（`GOLD_INITIAL`、`GOLD_MAX`、`REPUTATION_MIN`、`REPUTATION_MAX`）；`Start` 階段從 C-06 取得 `_currentMaxDebt` 初始值
...
4. `GetCurrentBankruptcyThreshold() : int` — 回傳快取的 `_currentMaxDebt`（來自 C-06 `GetMaxDebt()`）；C-06 危險度升級時由 `OnDangerLevelChanged` 事件觸發更新
...
6. C-06 `OnDangerLevelChanged` 觸發時，F-03 更新 `_currentMaxDebt`；由於危險度只升不降，`maxDebt` 只會更寬鬆（更負），不會觸發立即破產
...
GetCurrentBankruptcyThreshold():
    return _currentMaxDebt   // 快取自 C-06.GetMaxDebt()；由 OnDangerLevelChanged 事件更新

// C-06 危險度升級時呼叫
OnDangerLevelChanged(newLevel):
    _currentMaxDebt = C06WorldDangerSystem.GetMaxDebt()
    // 危險度只升不降，maxDebt 只會更寬鬆，不觸發立即破產
    // 執行 EvaluateWarningState() 以確保狀態一致性（防禦性）
    EvaluateWarningState()
```
...
| C-06 危險度升級，`maxDebt` 更寬鬆（更負） | 快取 `_currentMaxDebt` 更新，`currentGold` 仍在新門檻上方（門檻只變寬鬆），不觸發破產 |
...
| C-06 危險度升級（Warning 期間），`maxDebt` 更寬鬆 | 快取 `_currentMaxDebt` 更新，`currentGold` 仍高於新門檻，維持 `Warning`，倒數繼續 |
...
| 系統 | 用途 |
|------|------|
| F-01 DataManager | 讀取 `REPUTATION_MIN`、`REPUTATION_MAX`、`GOLD_MAX` 常數；查詢 `BankruptcyThresholdTable`（僅取 `warningDurationSec`）、`ReputationLabelTable` |
| F-02 Time System | 訂閱 `OnSecondTick` 驅動破產倒數檢查；訂閱 `OnOfflineResolved`（玩家確認離線摘要後）執行離線破產判定 |
| C-06 World Danger System | 查詢 `GetMaxDebt()` 作為金幣硬性下限（`_currentMaxDebt`）；訂閱 `OnDangerLevelChanged` 即時更新快取 | 
...
Resource Management 依賴 F-02 Time System 的 tick 事件，而 Time System 不依賴 Resource Management——**無循環依賴**。Resource Management 依賴 C-06 World Danger System 查詢 `maxDebt`，C-06 不依賴 F-03——**無循環依賴**。
...
### 7.2 破產警告期表（BankruptcyThresholdTable.csv）

每個聲望段設定破產警告倒數長度。金幣硬性下限（`maxDebt`）改由 C-06 `DebtLimitTable` 統一管控，不在此表設定。

| reputationMin | reputationMax | warningDurationSec | 說明 |
|---------------|---------------|-------------------|------|
| -100 | -1 | 86400 | 惡名聲望：24h 警告 |
| 0 | 19 | 172800 | 無名公會：48h 警告 |
| 20 | 39 | 259200 | 初具規模：72h 警告 |
| 40 | 59 | 259200 | 具名公會：72h 警告 |
| 60 | 79 | 345600 | 知名公會：96h 警告 |
| 80 | 100 | 604800 | 頂級公會：7d 警告 |

**調整原則**：
- `warningDurationSec`：安全範圍 `43200 ~ 604800`（12h ~ 7d）；過短玩家無法反應，過長失去張力
- 金幣下限見 C-06 `DebtLimitTable.csv`
...
| AC-RM-10 | 危險度升級更新債務上限 | 危險度由 E 升至 D，`OnDangerLevelChanged` 觸發後 `GetCurrentBankruptcyThreshold()` 回傳 `-500`（D 階 maxDebt）；原 `Warning` 狀態下 gold = -450（在新範圍內）→ 維持 `Warning`，不觸發 `Bankrupt` |
