...
| 4 | `[F-01] data-manager.md` | §6 登記 `Get<FactionRouteData>` / `Get<StoryStageData>` 兩個查詢路徑（`MissionFactionScoreWeightData` 已合併入 C-01 `MissionDifficultyTable`，由 C-01 owner 註冊） | **登記** |
...
| 9 | `[systems-index.md]` | `MissionFactionScoreWeight` 已合併入 C-01 `MissionDifficultyTable.factionScoreDelta`（systems-index 對應行於 2026-04-26 合併批次同步更新）；同步 `StoryStageTable` 命名（原 `StoryNodeTable`）；確認 FT-09 在系統索引內列為 Day 7-8 系統 | ✅ 已寫入(2026-04-26 design-review 修補 + 表格合併批次) |
