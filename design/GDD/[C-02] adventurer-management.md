...
| `currentMissionID` | `int` | 派遣中的任務 instanceID；非 `Dispatched` 狀態填 `0` |
...
| 更新狀態 | `UpdateStatus(int instanceID, AdventurerStatus newStatus, ...) : void` | FT-04 Outcome 呼叫 |
| 設定 Wounded | `SetWounded(int instanceID) : void` | 計算並寫入 `woundedUntilTimestamp`（當前時間 + `WOUNDED_RECOVERY_HOURS × 3600`）|
...
SetWounded(instanceID):
    now = F-02 TimeSystem.GetCurrentTimestamp()   // Unix 秒
...
| F-02 Time System | 取得當前 Unix timestamp，驅動 Wounded 恢復 Tick | `TimeSystem.GetCurrentTimestamp()`、Tick 回呼 |
