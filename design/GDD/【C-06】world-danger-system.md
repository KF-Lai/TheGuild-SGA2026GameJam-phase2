...
C-06 採用**單一資料表**集中所有「按危險度查找」的數值（升級三閘條件 + 任務池難度權重 + 破產債務上限），同 PK = `dangerLevel`、共 5 行（E / D / C / B / A），方便設計師單檔調整全部跨欄位平衡。

| 欄位群 | 欄位 | 型別 | 說明 |
|---|------|------|------|
| 主鍵 | `dangerLevel` | `string` (PK) | E / D / C / B / A |
| 顯示 | `name` | `string` | 顯示名稱（和平 / 動盪 / 暗湧 / 危局 / 末世） |
| 升級閘 | `timeThreshold` | `int` | 時間閘：升至此階所需最低遊戲天數 |
| 升級閘 | `missionCountReq` | `int` | 進度閘：累積接受任務數門檻 |
| 升級閘 | `minDifficulty` | `string` | 進度閘：計入 `acceptedMissionCount` 的最低任務難度 |
| 升級閘 | `factionScoreReq` | `int` | 陣營閘：任意陣營累積分數需達到此值；`0` = 不設此閘 |
| 任務池權重 | `weightF_E` | `int` | F~E 難度合計權重 |
| 任務池權重 | `weightD` | `int` | D 難度權重 |
| 任務池權重 | `weightC` | `int` | C 難度權重 |
| 任務池權重 | `weightB` | `int` | B 難度權重 |
| 任務池權重 | `weightA` | `int` | A 難度權重 |
| 任務池權重 | `weightS_SSS` | `int` | S~SSS 難度合計權重 |
| 債務上限 | `maxDebt` | `int` | 負值；金幣不可低於此值，由 F-03 Resource Management 強制執行 |

**Game Jam 初始資料：**

| dangerLevel | name | timeThreshold | missionCountReq | minDifficulty | factionScoreReq | weightF_E | weightD | weightC | weightB | weightA | weightS_SSS | maxDebt |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| E | 和平 | 0 | 0 | F | 0 | 40 | 30 | 20 | 8 | 2 | 0 | -100 |
| D | 動盪 | 1 | 10 | F | 0 | 20 | 35 | 25 | 15 | 5 | 0 | -500 |
| C | 暗湧 | 3 | 15 | D | 5 | 10 | 20 | 35 | 25 | 8 | 2 | -1000 |
| B | 危局 | 7 | 20 | C | 12 | 5 | 10 | 25 | 35 | 20 | 5 | -2500 |
| A | 末世 | 14 | 25 | B | 20 | 0 | 5 | 15 | 30 | 35 | 15 | -5000 |

> E 階為起始狀態，三閘均為 0，不需升級條件。E、D 階不設陣營閘（`factionScoreReq = 0`）；C 階起需至少一個陣營累積分數達到門檻，象徵世界格局開始被陣營勢力拉動。任務池權重與 `maxDebt` 隨危險度遞增，反映壓力梯度。
...
| `gameStartTimestamp` | `long` | 遊戲開始的 Unix timestamp（秒），用於計算遊戲天數；由 FT-10 Save/Load 在 `InitializeAsNewGame()`（新遊戲）寫入 `F02.NowUTC`，於 `RestoreFromSave()`（載入存檔）還原存檔值。寫入時機在 C-06 `Awake` 之後、`Start` 之前（FT-10 Bootstrap 流程保證；對應 §6.4 ISaveable 還原階段）|
...
| 任務池權重 | `GetPoolWeights() : MissionPoolWeights` | 回傳當前危險度的難度分布權重（取自 `WorldDangerTable` 該行的 `weightF_E` ~ `weightS_SSS` 欄位群） |
...
> **API 呼叫機制**：`OnMissionAccepted` 與 `OnFactionScoreUpdated` 為**直接 API 呼叫**（FT-02 / FT-09 直接呼叫 C-06），不走 EventBus，避免不必要的訂閱 / 發布開銷與時序耦合。`OnDangerLevelChanged` 為對外發布事件（走 EventBus，P-02 / FT-02 等下游訂閱）。實作層需明確區分：API 呼叫透過介面（`IWorldDangerService`），事件透過 `EventBus.Publish`。
...
```
GetElapsedDays():
    if gameStartTimestamp == 0:                  // 存檔損毀 / 未初始化（§5.2 row 2）
        Debug.LogError("gameStartTimestamp not initialized")
        return 0                                  // 視為時間閘未滿足，避免 (NowUTC - 0) / 86400 回傳極大值導致時間閘恆通過
...
> **`gameStartTimestamp == 0` 防護的必要性（2026-04-27 P-005 補強）**：若 `gameStartTimestamp` 因存檔損毀為 `0`，`(NowUTC - 0) / 86400` 會回傳約 19000+ 天（NowUTC 為 ~17 億秒），導致所有時間閘**恆滿足**——與 §5.2 row 2「視為時間閘未滿足」的設計意圖完全相反。此防護分支為硬性必要。
...
CheckLevelUp():
    ...
    if data == null:                              // §5.1 row 1：缺漏階跳過
        Debug.LogError("missing dangerLevel row: " + nextLevel)
        currentDangerLevel = nextLevel             // 直接視為已升至缺漏階，繼續嘗試下一階
        return CheckLevelUp()                      // 遞迴跳過該缺漏階
...
>
> **缺漏階跳過分支（2026-04-27 P-005 補強）**：若 `WorldDangerTable.csv` 缺某 dangerLevel 行（§5.1 row 1），`CheckLevelUp` 在 `GetDangerData(nextLevel) == null` 時 `Debug.LogError` 列出缺漏階別，直接設 `currentDangerLevel = nextLevel`（視為已升至缺漏階）並遞迴嘗試下一階。此分支讓 §5.1 row 1「升級跳過該階直接嘗試下一階」可被 Codex 照偽碼直接實作，無需揣測。

---

### 4.3 啟動時推送初始門檻至 F-03（方案A）
...
>
> **實作方式（2026-04-27 P-005 補強）**：兩者皆以 `[DefaultExecutionOrder(int)]` 屬性宣告於 MonoBehaviour 類別上（F-03 較小、C-06 較大，例如 F-03 = 50、C-06 = 100），程式級保證、可被版本控制，**不依賴 Unity Editor → Project Settings → Script Execution Order 的人工設定**。理由：人工設定未版本控制、新人易踩雷、跨機器易遺漏；屬性式為硬性執行順序保證。
...
OnFactionScoreUpdated(newMaxScore):
    if currentDangerLevel == "A":
        return                                    // 已最高階，無需重檢；對齊 §5.2 row 5「A 階呼叫不報錯」精神（2026-04-27 P-005 補強）
...
| 該行 `weightF_E` ~ `weightS_SSS` 任一缺失 / 全為 0 | `Debug.LogError`，`GetPoolWeights()` 回傳 E 階權重作為 fallback |
| 該行 `maxDebt` 缺失 / 為 0 | `Debug.LogError`，`GetMaxDebt()` 回傳 `-100`（E 階預設值）作為 fallback |
...
| 系統 | 依賴內容 | 介面 |
|------|---------|------|
| F-01 DataManager | 載入 `WorldDangerTable`（單一資料表，內含升級閘 / 任務池權重 / 債務上限三組欄位） | `DataManager.GetAll<WorldDangerData>()` |
...
### 7.2 WorldDangerTable.csv（任務池權重）

| 參數 | 安全考量 |
|------|---------|
| 各危險度的難度權重分布（`weightF_E` ~ `weightS_SSS`） | `weightS_SSS` 在 E / D 階維持 `0`，避免玩家開局就面對 S 難度任務；A 末世的 `weightF_E` 設為 `0`，強制玩家面對高難度委託 |

---

### 7.3 WorldDangerTable.csv（債務上限）
