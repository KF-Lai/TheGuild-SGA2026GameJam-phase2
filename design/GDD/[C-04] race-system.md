...
C-04 Race System 定義遊戲中所有冒險者種族的靜態資料，以 `RaceTable` 為核心，儲存每個種族對特定任務類型的成功率修正（`successModifiers`）與死亡率修正（`deathModifiers`）。`ProfessionRacePool` 定義每個職業對應的種族隨機池與權重，供 C-02 Adventurer Management 在冒險者生成時（`raceID = 0`）呼叫 `RollRace(professionID)` 抽取種族。系統架構在 F-01 DataManager 之上，提供種族查詢 API；修正值的套用由 FT-02 Mission Dispatch 負責，C-04 僅提供原始資料。C-04 不持有任何 runtime 狀態，亦不感知任何冒險者實例。
...
### 3.2 ProfessionRacePool 資料表定義

每個職業對應一組種族隨機池，冒險者生成時（`raceID = 0`）依此加權抽取。

| 欄位 | 型別 | 說明 |
|------|------|------|
| `professionID` | `int` (PK, FK → ProfessionTable) | 職業 ID |
| `raceIDs` | `int[]`（`\|` 分隔） | 可抽取的種族 ID 列表 |
| `weights` | `int[]`（`\|` 分隔） | 對應每個 raceID 的相對權重；長度必須與 `raceIDs` 一致 |

**Game Jam 初始資料（7 種職業）：**

| professionID | name | raceIDs | weights | 設計意圖 |
|-------------|------|---------|---------|---------|
...
| 加權抽種族 | `RollRace(int professionID) : int` | 依 ProfessionRacePool 加權隨機，回傳 `raceID` |
...
RollRace(professionID):
    pool = DataManager.Get<ProfessionRacePool>(professionID)
    if pool == null → Debug.LogError, return 1  // fallback 人類（raceID=1）

    totalWeight = pool.weights.Sum()
    roll = Random.Range(0, totalWeight)          // [0, totalWeight)
    cumulative = 0
    for i in 0..pool.raceIDs.Length:
        cumulative += pool.weights[i]
        if roll < cumulative:
            return pool.raceIDs[i]

    return pool.raceIDs.Last()                   // 防禦性 fallback（理論上不到達）
```
...
| `ProfessionRacePool` 的 `raceIDs` 與 `weights` 長度不一致 | `Debug.LogError`，該職業的隨機池視為無效，`RollRace` 對此職業回傳 fallback `raceID=1` |
| `ProfessionRacePool` 的 `raceIDs` 包含不存在於 RaceTable 的 raceID | `Debug.LogWarning`，過濾該 raceID 與對應 weight，其餘正常抽取 |
| `ProfessionRacePool` 過濾後 `raceIDs` 為空 | `Debug.LogError`，`RollRace` 回傳 fallback `raceID=1` |
| 某職業在 `ProfessionRacePool` 中無對應行 | `Debug.LogError`，`RollRace` 回傳 fallback `raceID=1` |
...
| 系統 | 依賴內容 | 介面 |
|------|---------|------|
| F-01 DataManager | 載入 `RaceTable`、`ProfessionRacePool` | `DataManager.GetAll<RaceData>()`、`DataManager.GetAll<ProfessionRacePool>()` |
| C-01 Mission Database | 提供合法 typeID 集合，供 modifiers 載入時驗證 | `MissionDatabase.GetAllMissionTypes()` |
| C-03 Profession System | 提供合法 professionID 集合，供 ProfessionRacePool 載入時驗證 | `ProfessionSystem.GetAllProfessions()` |
...
- C-04 依賴 C-03 進行 professionID 驗證，C-03 不依賴 C-04——**無循環依賴**
...
| 新增種族 | — | 在 CSV 新增一行並分配唯一 `raceID`，同時在 `ProfessionRacePool` 各職業池中加入新 `raceID` 與 `weight` |

---

### 7.2 ProfessionRacePool.csv

| 調整項目 | 安全範圍 | 影響 |
|---------|---------|------|
| 各職業種族權重分布 | weights 總和無需為 100，相對比例即可 | 調整職業與種族的視覺關聯性；偏斜過大（如 95:5）會讓該職業幾乎只出現一種種族，降低名冊多樣性 |
...
| AC-RS-12 | `ProfessionRacePool` 中 `raceIDs` 與 `weights` 長度不一致時，啟動出現 `LogError`，該職業 `RollRace` 回傳 fallback `1` |
