...
| AC-8 | `CommissionBreakdown` 所有 20 欄非預設空值；非 clamp 情境下 `goldBefore + netDelta == goldAfter` | EditMode 斷言測試 |
| AC-9 | 維護費 `totalAmount=95`，金幣 `500→405`；defensive copy 驗證：上游修改 dict 不影響 breakdown | EditMode 測試 |
| AC-10 | 薪水 `totalAmount=90`，金幣 `500→410`；`SalaryBreakdown.items` 含 2 項 | EditMode 測試 |
| AC-11 | `totalAmount ≤ 0` 或 `items=null` → 金幣不變、未發布 `Charged` 事件、console error | EditMode 測試 |
...
**B-01（建議項）**：GDD §5.5.3 提及「F-03 保證單一 `netDelta` 無法同時穿零線兩次」，但 F-03 GDD 目前無明確條文說明此保證。建議撰寫 F-03 FSD 或 GDD 回顧時補充此邊界保證，避免未來 F-03 實作變動時隱性破壞 FT-05 的狀態快照語義。此項不阻礙 FT-05 實作啟動。
