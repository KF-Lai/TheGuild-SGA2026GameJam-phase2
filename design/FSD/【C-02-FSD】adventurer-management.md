...
| AddAdventurer | `AddAdventurer(AdventurerInstance instance) : bool` | **rosterCap 防守由呼叫端負責**（FT-01 在呼叫前先用 `IsRosterFull(rosterCap)` 檢查；FSD-Codex-Reoprts-260427 T1-C02 裁決）；本方法只做 isUnique 衝突防守，衝突時回傳 `false`。AC-AM-07 改測呼叫端 `IsRosterFull` 行為 |
...
FT-01.TryRecruitFromTemplate(templateID)
  → AdventurerFactory.CreateFromTemplate(templateID)
      ...
      ├─ 3：instance = new AdventurerInstance { instanceID = IAdventurerRoster.AllocateInstanceID(), templateID, name, rank, professionID }
  ...
  → IAdventurerRoster.AddAdventurer(instance)
      // T1-C02 裁決：rosterCap 防守由呼叫端負責，AddAdventurer 不再持有 rosterCap 參數
      ├─ 1：if instance.templateID != 0 && isUnique 衝突（含 Dead）→ LogWarning, return false
      ├─ 2：roster.Add(instance)
      ├─ 3：if instance.status == Idle → instance.idleSinceTimestamp = ITimeSystem.NowUTC
      ├─ 4：EventBus.Publish(new OnAdventurerAddedEvent(instance.instanceID))   // T1-C02 新增 typed event
      └─ 5：return true
```
...
### 6.4 ISaveable 持久化契約（FSD-Codex-Reoprts-260427 T1-C02 補完）

**Owner**：`AdventurerRoster` 為唯一 ISaveable owner。`AdventurerFactory` / `AdventurerTemplateLoader` / `AdventurerWoundedRecovery` 不持久化（前者無狀態；後二者由 CSV 與訂閱回填）。

| 欄位 | 型別 | 說明 |
| --- | --- | --- |
| `OwnerKey` | string 常數 | `"c02AdventurerRoster"` |
| `IsCritical` | bool | `true`（名冊損毀視為存檔不可用） |
...
[Serializable]
class C02SaveDTO {
    public List<AdventurerInstance> roster;   // 含所有狀態（Idle/Dispatched/Wounded/Dead）
    public int                       _nextInstanceID;
}
```

**Serialize() 流程**：
1. 取 `_roster` 全部 entries（不過濾 Dead）。
2. 寫入 `_nextInstanceID` 當前值。
3. 序列化為 JSON。

**RestoreFromSave(json) 流程**（驗證失敗分流）：

| 失敗類型 | 處理方式 |
| --- | --- |
| `templateID` 找不到（模板已從 CSV 移除） | **保留實例**；`templateID` 改寫為 `0`（隨機生成 sentinel）；`LogWarning` |
| `professionID` 在 ProfessionTable 找不到 | **跳過該實例**（resilience 優先於數量）；`LogError` |
| `raceID` 非 0 但 RaceTable 找不到 | **保留實例**；`raceID` 改寫為 `1` fallback（與 C-04 對齊）；`LogWarning` |
| 單一 `traitID` 在 TraitTable 找不到 | **過濾該 traitID**，保留其餘；`LogWarning` |
| `status` 為非法 enum 值 | **跳過該實例**；`LogError` |
| `currentMissionID != 0` 但對應 ActiveMission 不存在（待 FT-02 RestoreFromSave 後驗證） | **狀態降回 Idle**、清空 `currentMissionID`；`LogWarning` |
| `_nextInstanceID < max(roster.instanceID) + 1` | 自動修正為 `max(roster.instanceID) + 1`；`LogWarning` |

**InitializeAsNewGame()**：`_roster = new List<>(); _nextInstanceID = 1;`

**Restore Order**：C-02 必須在 C-01 / C-03 / C-04 / C-05 完成後 restore（依賴職業/種族/特質表已載入）；FT-02 在 C-02 之後（依賴 roster 來驗證 `currentMissionID`）；FT-10 RestoreOrchestrator 排程責任。
...
- **B-03（CT-05 裁決確認，FSD-Codex-Reoprts-260427）**：明確 C-02 介面切分為 `IAdventurerFactory`（建構：`CreateFromTemplate` / `CreateRandomInstance`）與 `IAdventurerRoster`（CRUD + 狀態：`AddAdventurer` / `IsRosterFull` / `GetRoster` / `GetByStatus` / `UpdateStatus` / `SetWounded` / `DismissAdventurer` / `TickWoundedRecovery` / `GetRecruitCost` / `SetLastAutoPickupTimestamp`）。`BuildTraitList` 為 `AdventurerFactory` 內部 helper，**不對外公開**。FT-01 / FT-02 / FT-04 等下游 FSD 不可呼叫 `IAdventurerRoster.BuildTraitList`；隨機冒險者由呼叫端先抽完 `traitIDs`（透過 C-05 `GetProfessionGroups` + `RollTraits`）後傳入 `CreateRandomInstance`，具名模板由 `CreateFromTemplate(templateID)` 內部組合 `template.fixedTraitIDs` + `template.randomTraitGroupIDs`。
