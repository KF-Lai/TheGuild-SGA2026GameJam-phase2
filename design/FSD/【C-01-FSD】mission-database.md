...
- 14 個 GDD §3.3 定義的查詢 API（含 `GetAllMissionTypes` 公開 API；GDD 內文寫「13 個」為計數疏漏，FSD-Codex-Reoprts-260427 T1-C01 修正）
...
| 系統 | 依賴內容 |
| --- | --- |
| F-01 DataManager | `RegisterTable<T>` 註冊四張機制表 + `GetAll<T>()` 載入時建立內部字典；`PickRandomWhere<MissionNameData>` 查詢 D-02 `MissionNamePool`；`GetInt`/`GetFloat` 取得 SystemConstants |
| FT-09 Faction Story（**FSD 未存在**，prerequisite） | `FactionRouteTable.csv` owner = FT-09；C-01 §5.4 / §7 在 Loader 階段驗證 `factionID != FACTION_NEUTRAL_ID` 時是否在 `FactionRouteTable` 內。**FT-09 FSD 未完成前，C-01 Loader 跳過此 FK 驗證**（直接視為合法），於 FT-09 FSD 與 FactionRouteTable Data-Specs 完成後回頭啟用驗證；目前 §5.4 偽碼保留驗證流程作為實作預留。 |
...
GDD §3.3 定義 14 個 API（GDD 內文寫「13 個」屬計數疏漏，實際包含 `GetAllMissionTypes`），職責涵蓋：(A) 資料載入與 FK 驗證、(B) 一般查詢分發、(C) 護送時長計算（依賴 SystemConstants 隨機邏輯）、(D) D-02 文字 Facade（依賴 `PickRandomWhere` 與 fallback 邏輯）。若集中於單一類別：
- 職責超過 1 種，違反 SRP。
...
MissionDatabaseLoader.LoadAll()
  → IDataManager.GetAll<MissionDifficultyData>()
      ├─ 建立 _difficultyDict: Dictionary<string, MissionDifficultyData>（以 difficulty 為 key）
      ├─ foreach row: 若 baseDeathRate 不在 [0.0,1.0] → LogError + Clamp
      └─ 若 factionScoreDelta < 0 → LogError + 設為 0

  → IDataManager.GetAll<MissionTypeData>()
      └─ 建立 _typeDict: Dictionary<int, MissionTypeData>（以 typeID 為 key）
         若 list 完全空白 → LogError

  → IDataManager.GetAll<MissionCategoryData>()
      └─ 建立 _categoryDict: Dictionary<int, MissionCategoryData>（以 categoryID 為 key）
         若 list 完全空白 → LogError

  → IDataManager.GetAll<MissionTemplate>()
      └─ foreach row:
...
| §4.1 GDD 範例 D=150 推導 450~750 | 採 GDD §3.1 與 AC-MD-07 的 **D=30**（`baseDuration=30`）為準，輸出範圍 90~150 | **回註**（FSD-Codex-Reoprts-260427 T1-C01）：GDD §4.1 範例文字寫「D baseDuration=150 → 450~750」與 §3.1 表格 / AC-MD-07 的 30 → 90~150 衝突；FSD 採資料表側為真值，§4.1 範例值待 GDD 修訂同步 |
...
- [x] §5.1~§5.4：14 個 API 簽名全列出；事件清單（無）說明；4 個 DTO 資料結構定義；5 個觸發點資料流偽碼齊備
