...
- `OnDangerLevelChangedEvent` 事件發布（payload：`string newDangerLevel`）
...
| DoD-12 | `OnDangerLevelChangedEvent` 每次升階觸發 1 次帶正確 `string newDangerLevel`（AC-WD-14） | EditMode test：訂閱事件計次 |
...
| 系統 | 依賴內容 |
| --- | --- |
| F-01 DataManager | `RegisterTable<WorldDangerData>("WorldDangerTable")` 於 `[RuntimeInitializeOnLoadMethod(BeforeSceneLoad)]` 註冊；`GetAll<WorldDangerData>()` 取得所有 5 階資料；C-06 Loader 自建 `_dangerDict: Dictionary<string, WorldDangerData>` 以 `dangerLevel` 為 key |
| F-02 Time System | `NowUTC` 計算 `elapsedDays`；訂閱 typed `OnDailyResetEvent` 觸發 `CheckLevelUp` |
...
| P-02 Main UI | 訂閱 typed `OnDangerLevelChangedEvent` 即時更新 UI；呼叫 `GetCurrentLevel` / `GetDangerData` 顯示當前階別與描述 |
...
| --- | --- | --- | --- |
| `OnDangerLevelChangedEvent` | Out（C-06 → *） | `(string newDangerLevel)` | `CheckLevelUp` 升階成功後立即發布；P-02 訂閱更新 UI；FT-02 訂閱觸發 §3.9.2 「危險度升階補池」流程 |
| `OnDailyResetEvent` | In（F-02 → C-06） | 空 payload（與 F-02 FSD §5.2 一致） | F-02 每日 00:00 發布；C-06 訂閱呼叫 `CheckLevelUp` 嘗試升階 |
...
| --- | --- | --- |
| 時代壓力悄悄逼近 | 委託板逐漸出現高難度任務 | 升階時發布 typed `OnDangerLevelChangedEvent`（payload：`string newDangerLevel`），FT-02 §3.9.2 「危險度升階補池」依新 `GetPoolWeights()` 重抽 |
...
| `WorldDangerService` | `Assets/Scripts/Gameplay/WorldDanger/WorldDangerService.cs` | MonoBehaviour：持有 4 個 runtime 狀態、實作 7 個 API、執行 `CheckLevelUp` 遞迴、Awake/Start 推送、訂閱 typed `OnDailyResetEvent`、實作 `ISaveable` | `WorldDangerLoader`（組合）、`ITimeService`、`IResourceService`、`EventBus` | `200~270 行` |
| `WorldDangerEvents` | `Assets/Scripts/Gameplay/WorldDanger/Events/WorldDangerEvents.cs` | 集中宣告 typed event struct：`OnDangerLevelChangedEvent { string newDangerLevel; }` | — | `< 50 行` |
...
WorldDangerService（MonoBehaviour）
  ...
  ├─ → ITimeService（NowUTC、訂閱 typed OnDailyResetEvent）
  ├─ → IResourceService（呼叫 SetBankruptcyThreshold；C-06 → F-03 單向推送）
  ├─ → EventBus（發布 OnDangerLevelChangedEvent）
...
| `void CheckLevelUp()` | F-02 `OnDailyResetEvent` 訂閱 callback；亦由 `OnMissionAccepted` / `OnFactionScoreUpdated` 內部觸發；A 階 no-op；遞迴自我呼叫直到無法升階 | 無錯誤路徑；缺漏階透過 fallback 跳過 |

### 5.2 事件清單

| 事件名稱 | 方向 | Payload | 發布時機 / 訂閱目的 |
| --- | --- | --- | --- |
| `OnDangerLevelChangedEvent` | Out（C-06 → *） | `(string newDangerLevel)` | `CheckLevelUp` 升階成功後立即發布；P-02 訂閱更新 UI；FT-02 訂閱觸發 §3.9.2 危險度升階補池 |
| `OnDailyResetEvent` | In（F-02 → C-06） | 空 payload | F-02 每日 00:00 發布；C-06 在 `OnEnable` 訂閱、`OnDisable` 取消訂閱；訂閱 callback 直接呼叫 `CheckLevelUp` |
...
WorldDangerService.Awake()
  → WorldDangerLoader.LoadAll()
      ├─ IReadOnlyList<WorldDangerData> entries = IDataManager.GetAll<WorldDangerData>()
      ├─ foreach entry:
...
**訂閱生命週期（FSD-Codex-Reoprts-260427 T1-C06 修正）**：

```
WorldDangerService.OnEnable()
  → EventBus.Subscribe<OnDailyResetEvent>(HandleDailyReset)
WorldDangerService.OnDisable()
  → EventBus.Unsubscribe<OnDailyResetEvent>(HandleDailyReset)
```

> 訂閱必須在 `OnEnable` / `OnDisable` 配對處理（不在 `Start` 訂閱、不在 `OnDestroy` 解除）；避免 disable→enable 切換時重複訂閱或漏退訂。`Start()` 只負責對 F-03 的初始 `SetBankruptcyThreshold` 推送，不涉及事件訂閱。
...
WorldDangerService.OnMissionAccepted(difficulty)
  ...
  // FSD-Codex-Reoprts-260427 T1-C06：補 TryGetValue 防護非法 difficulty
  → if !DifficultyIndex.TryGetValue(difficulty, out int acceptedIndex)
       → LogWarning($"OnMissionAccepted received invalid difficulty: {difficulty}"), return
  → if !DifficultyIndex.TryGetValue(nextData.minDifficulty, out int requiredIndex)
       → LogError($"WorldDangerTable.minDifficulty invalid: {nextData.minDifficulty}"), return
  → if acceptedIndex >= requiredIndex:
       ├─ _acceptedMissionCount++
       └─ CheckLevelUp()
```

**觸發點 4：F-02 OnDailyResetEvent 訂閱 callback**

```
F02.OnDailyResetEvent → WorldDangerService.HandleDailyReset()
  → CheckLevelUp()
```
...
WorldDangerService.RestoreFromSave(json)
  ...
  → 不立即觸發 CheckLevelUp（保留存檔當下狀態；下一次 OnDailyResetEvent / API 呼叫會自然觸發）
```
...
| §3.3 升級規則 rule 7（推送 + 發布事件） | §5.4 觸發點 6 `SetBankruptcyThreshold` + `OnDangerLevelChangedEvent` | 對齊 | 升階原子操作三步驟 |
...
- [x] §2.1~§2.5：GDD 章節（10 條）、Data-Specs（1 份）、上游（2 系統）、下游（5 系統）、事件契約（typed `OnDangerLevelChangedEvent` out + typed `OnDailyResetEvent` in）皆列舉
