...
- 離線時差計算與 `OnOfflineResolvedEvent` 事件發布。
- 每日重置事件（`OnDailyResetEvent`）跨日偵測（即時與啟動兩條路徑）。
...
6. **每日重置測試**：mock UTC 日期跨越，`OnDailyResetEvent` 發布恰一次。
...
| FT-01 Adventurer Recruitment  | `OnDailyResetEvent` 重置免費刷新次數                                    |
| C-06 World Danger System      | `OnDailyResetEvent` 觸發 `CheckLevelUp()`；`NowUTC` 計算 `GetElapsedDays()` |
...
| `OnDailyResetEvent` | F-02 → * | 空 struct（無欄位）                            | UTC `DAILY_RESET_HOUR`:00 跨越；下游重置每日狀態 |
| `OnOfflineResolvedEvent` | F-02 → FT-02 / F-03 | `long offlineSeconds`                | `Initialize` 計算完成；FT-02 推進任務並發 `OnOfflineMissionsResolved`、F-03 判定離線破產 |
...
| 時間真實流逝（離線）             | 關遊戲一段時間後重啟，看到「離開了 X 小時 Y 分鐘」摘要畫面 | `Initialize(lastActiveTimestamp)` 計算 `nowUTC - lastActiveTimestamp`，發 `OnOfflineResolvedEvent`           |
...
| 每日節律（招募刷新、世界升階）   | 過了凌晨 0 點，招募板免費刷新次數歸零                      | 啟動時比對 UTC 日期跨越；即時模式偵測 `DAILY_RESET_HOUR`:00 跨越；發 `OnDailyResetEvent`                     |
...
| `GameEvents`        | `Assets/Scripts/Core/Events/GameEvents.cs`                      | 集中定義 `OnSecondTickEvent` / `OnMinuteTickEvent` / `OnDailyResetEvent`（空 struct）/ `OnOfflinePendingEvent` / `OnOfflineResolvedEvent` 的 payload struct（避免裝箱）。 | —                                       | < 50 行   |

> 註：既有檔案 `Assets/Scripts/Core/Time/MissionTimer.cs` 已由 CT-03 裁決移除（職責由 FT-02-A `MissionDispatchService.TickCompletionCheck` 取代）。本 FSD 不規劃任務計時器；§8.3 第 2 條登記移除範圍。
...
| `void Initialize(long lastActiveTimestamp)`  | Save/Load 載入完成後呼叫；計算 `offlineSeconds = clamp(NowUTC - lastActiveTimestamp, 0, OFFLINE_MAX_SECONDS)`，發 `OnOfflineResolvedEvent`；同時觸發跨日偵測補發 `OnDailyResetEvent`（若跨越）。 |
...
> `_tickPaused == true` 後，所有事件停止發布（包含 `OnSecondTick` 連帶的 `OnMinuteTick` 與 `OnDailyResetEvent`）。`NowUTC` 不受影響。
...
| §5.1 `offlineSeconds > OFFLINE_MAX_SECONDS`               | `Initialize` 內以 `Math.Min(raw, _offlineMaxSeconds)` 截上限；正常發 `OnOfflineResolvedEvent`                                              | `TimeSystem`  | EditMode：mock 8 天離線，斷言 `offlineSeconds == 604800`         |
| §5.1 首次啟動（無 `lastActiveTimestamp`）                  | FT-10 傳入 `lastActiveTimestamp = 0`；`Initialize` 偵測 `lastActiveTimestamp == 0` 時直接設 `offlineSeconds = 0`、不跨日偵測          | `TimeSystem`  | EditMode：傳 0，斷言 `offlineSeconds == 0` 且 `OnDailyResetEvent` 不發 |
...
| §5.2 任務派遣後立即關閉再開啟                              | `Initialize` 觸發 `OnOfflineResolvedEvent`，FT-02 收到後依 `dispatchTimestamp + durationSeconds - NowUTC` 重算（採目標時間戳，不累積誤差）  | `TimeSystem`  | FT-02 EditMode：派遣後立即離線 3600 秒，斷言任務剩餘秒數正確     |
| §5.2 結算瞬間關閉遊戲                                     | 與上同；`remainingSeconds <= 0` 由 FT-02 在 `OnOfflineResolvedEvent` 訂閱端判定                                                            | （無，跨系統） | FT-02 EditMode 測試                                              |
| §5.3 離線跨越多個 UTC 00:00                                | `Initialize` 比對 `(年, dayOfYear)` 變化，**只發一次** `OnDailyResetEvent`；不論跨幾天都只發一次（重置是狀態，不是計數）              | `TimeSystem`  | EditMode：mock 3 天離線，斷言 `OnDailyResetEvent` 被發布恰 1 次       |
| §5.3 玩家開著遊戲跨 UTC 00:00                              | `Update()` 每秒比對 `(_lastResetUtcYear, _lastResetUtcDayOfYear)` 與當前；跨越且 `Hour >= _dailyResetHour` 時即時發 `OnDailyResetEvent`     | `TimeSystem`  | EditMode：mock 系統時間跨日，斷言 `Update()` 在跨越秒內發 `OnDailyResetEvent` |
| §5.3 每日重置與離線結算同時發生                            | `Initialize` 內**先**發 `OnOfflineResolvedEvent`，**後**發 `OnDailyResetEvent`（順序固定，不依賴訂閱者執行順序）                                  | `TimeSystem`  | EditMode：mock 3 天離線，斷言 `OnOfflineResolvedEvent` 先於 `OnDailyResetEvent` |
...
| §3.7 `PauseTick` 後跨 UTC 00:00                            | `Update()` 早退（`if _tickPaused: return`），不發 `OnDailyResetEvent` 也不發 `OnSecondTickEvent`                                                | `TimeSystem`  | EditMode：呼叫 `PauseTick`，mock 跨日，斷言 `OnDailyResetEvent` 不發  |
...
| §3.3.4 對所有任務 `remainingSeconds -= offlineSeconds` | §5.4 流程備註            | 對齊     | 由 FT-02 訂閱 `OnOfflineResolvedEvent` 後執行（採方案 A）         |
...
2. **既有 Script 偏差（已裁決移除，FSD-Codex-Reoprts-260427 CT-03）**：`Assets/Scripts/Core/Time/MissionTimer.cs` 存在於 F-02 目錄，且 `TimeSystem.cs` 持有 `_missionTimers` 欄位、`RegisterMission` / `UnregisterMission` / `CheckMissionTimers` / `PublishMissionExpired` 方法。CT-03 已裁決：**任務計時 owner 唯一歸屬 FT-02-A `MissionDispatchService.TickCompletionCheck`**；F-02 不再持有任務計時器。實作 PR 移除範圍：(a) 刪除 `MissionTimer.cs`；(b) `TimeSystem.cs` 移除上述欄位/方法、`ConfirmOfflineResolution` 內 `PublishMissionExpired` 發布迴圈；(c) `GameEvents.cs` 移除或標 `[Obsolete]` `OnMissionExpiredEvent` struct；(d) `OfflineSummary` 移除 `CompletedMissionInstanceIds` / `CompletedCount`。本 FSD §5.4 既有 `RegisterMission` / `UnregisterMission` API 已不存在，無需 §5 章節更新（過去版本曾列入但本 FSD 已不規劃）。
...
4. **既有 Script Drift — `OnDailyResetEvent` event 型別不一致（FSD-Codex-Reoprts-260427 CT-02 已裁決）**：本 FSD §5.2 / §5.3 / §5.4 規定 typed `OnDailyResetEvent` 結構發布；但 `Assets/Scripts/Core/Time/TimeSystem.cs` (line 105 / 411) 目前仍呼叫 `EventBus.Publish(EventNames.OnDailyReset)`（named string event），且 `Assets/Scripts/Core/Events/GameEvents.cs` 缺 `OnDailyResetEvent` struct 定義。決議：採 typed event（與 `OnSecondTickEvent` / `OnMinuteTickEvent` 模式一致）；**待實作 PR 處理**：(1) `GameEvents.cs` 新增 `public readonly struct OnDailyResetEvent { }`；(2) `TimeSystem.cs` 兩處 `Publish(EventNames.OnDailyReset)` 改為 `Publish(new OnDailyResetEvent())`；(3) `EventNames.OnDailyReset` 字串常數標 `[Obsolete]` 或刪除。下游 FSD（C-06、FT-01、FT-02-B）已同步更新為 typed 訂閱。

### 8.4 給 GDD 的回註紀錄

| 日期 | GDD 檔案 | 章節 | 回註摘要 |
| --- | --- | --- | --- |
| 2026-04-26 | `【F-02】time-system.md` | §6.3 表後（`OnOfflineResolvedEvent` 列） | 採方案 A：`OnOfflineResolvedEvent` payload 僅含 `offlineSeconds`；`completedCount` 改由 FT-02 `OnOfflineMissionsResolved` 提供；§4.3 `completedCount` 改由 FT-02 提供 |
| 2026-04-26 | `【FT-02】mission-dispatch.md` | §6.1 表後 | FT-02 額外承擔：訂閱 F-02 `OnOfflineResolvedEvent` → 計算 `completedCount` → 發 `OnOfflineMissionsResolved(offlineSeconds, completedCount, completedActiveMissionIDs[])`；不修改 §3.7 既有規則，待 FT-02 FSD 細化 |
