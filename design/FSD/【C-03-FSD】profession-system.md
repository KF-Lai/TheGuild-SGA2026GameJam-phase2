...
C-03 Profession System 負責從 `ProfessionTable.csv` 載入職業靜態資料、執行資料完整性驗證，並對外提供職業查詢 API。系統完全資料驅動：新增職業只需在 CSV 加行，程式碼無需改動。系統不持有任何 runtime 狀態。**不可變策略**：`ProfessionData` 在 Loader 完成驗證後 freeze 集合（`StrongTypeIds` 為 `IReadOnlySet<int>`，`RaceIDs` / `RaceWeights` / `TraitGroupIDs` 為 `IReadOnlyList<>`），下游無法修改靜態資料；CSV 欄位名（`raceIDs`、`strongTypeIDs` 等）與 C# 屬性名（`RaceIDs`、`StrongTypeIDs`，PascalCase + ID 大寫）由反射綁定 mapping。
...
| DoD-09b | **AC-PS-12 正向測試**：注入合法 tier=2 職業（professionID=8, baseProfessionID=1），`GetUpgradePaths(1)` 回傳含 8 的列表；`GetBaseProfession(8)` 回傳 professionID=1（FSD-Codex-Reoprts-260427 T1-C03 補） | EditMode test |
| DoD-09c | **CSV 行序測試**：注入 tier=2 行排在 base profession 行之前的 CSV，Loader 仍能正確驗證 baseProfessionID 合法性（兩階段驗證確保） | EditMode test |
| DoD-09d | **raceWeights 長度驗證**：注入 raceWeights.Length != raceIDs.Length 的職業，Loader `Debug.LogError` 且該職業不入字典；C-04 不再重複驗證 | EditMode test |
| DoD-10 | CSV 零硬編碼：`STRONG_TYPE_BONUS`/`WEAK_TYPE_PENALTY` 來自 `SystemConstants.csv` | 程式碼 review：無魔術數字 0.20 / 0.15 |
| DoD-11 | **不可變性**：`GetProfession(id).RaceIDs` / `RaceWeights` / `StrongTypeIds` / `TraitGroupIDs` 回傳唯讀集合（`IReadOnlyList<>` / `IReadOnlySet<>`），下游嘗試呼叫 `Add` / `Clear` 會編譯錯誤 | 程式碼 review |
...
| C-01 Mission Database | 提供合法 typeID 集合，供載入時驗證 `strongTypeIDs`/`weakTypeIDs` | `IMissionDatabaseService.GetAllMissionTypes()` 回傳 `IReadOnlyList<MissionTypeData>`（C-03 Loader 自行投影 `typeID`；FSD-Codex-Reoprts-260427 T1-C03 修正：原寫 `GetAllMissionTypeIds()` 為非既有 API） |
...
Bootstrap.Initialize()（F-01 + C-01 完成後執行）
  → ProfessionDatabaseLoader.Initialize(dataManager, missionDb)
      ├─ 步驟 1：rawList = dataManager.GetAll<ProfessionData>() 取得原始列表
      ├─ 步驟 2：validTypeIds = HashSet from missionDb.GetAllMissionTypes().Select(t => t.typeID)
      │         // FSD-Codex-Reoprts-260427 T1-C03 修正：改用 C-01 既有 API GetAllMissionTypes()
      │         // 而非不存在的 GetAllMissionTypeIds()
      ├─ 步驟 3a（兩階段驗證 Phase 1）：建立全部 professionID 集合
      │         allProfessionIds = new HashSet<int>(rawList.Select(p => p.professionID))
      │         // T1-C03 修正：避免 CSV 行序影響 baseProfessionID 驗證合法性
      ├─ 步驟 3b（兩階段驗證 Phase 2）：對每筆 rawData 執行驗證：
      ...
      │   │     → Debug.LogError($"professionID={id} raceWeights.Length({weights}) != raceIDs.Length({ids})")
      │   │     → 跳過（不加入字典）
      │   │     // T1-C03 補：依 GDD §3.1 / §5.1 跨欄位驗證；C-03 為 owner（C-04 不再重複驗證）
      │   ├─ foreach w in raceWeights: if (w <= 0) → Debug.LogError + 跳過
      │   │     // T1-C03 補：raceWeights 非正值由 C-03 防守
      │   ├─ if (tier >= 2 && baseProfessionId == 0)
      │   │     → Debug.LogError($"professionID={id} tier≥2 但 baseProfessionID=0")
      │   │     → 跳過
      │   ├─ if (tier >= 2 && !allProfessionIds.Contains(baseProfessionId))
      │   │     → Debug.LogError($"professionID={id} baseProfessionID={baseProfessionId} 不存在")
      │   │     → 跳過
      │   │     // T1-C03 修正：用 allProfessionIds（Phase 1 結果）而非 dict.ContainsKey
      ...
      │       // 不可變策略：ProfessionData 在加入 dict 前 freeze 集合
      │       //   StrongTypeIds → IReadOnlySet<int>
      │       //   RaceIDs / RaceWeights / TraitGroupIDs → IReadOnlyList<int>/<float>
