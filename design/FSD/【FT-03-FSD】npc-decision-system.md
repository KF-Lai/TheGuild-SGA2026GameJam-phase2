...
| 系統 | 依賴內容 | 使用介面 |
| --- | --- | --- |
| F-01 DataManager | 載入 SystemConstants 常數 | `IDataManager.GetFloat(key)` |
| F-02 Time System | 取得當前 Unix 秒時間戳；訂閱 `OnMinuteTick` 驅動 `AutoPickupTick` | `ITimeSystem.NowUTC`、`EventBus.Subscribe<OnMinuteTickEvent>` |
...
| FT-10 Save/Load | `idleSinceTimestamp` / `lastAutoPickupTimestamp` 透過 C-02 AdventurerInstance 一併序列化；**FT-03 不實作 ISaveable**（FSD-Codex-Reoprts-260427 T1-FT03 裁決：FT-03 無自有持久化狀態；移除 `OwnerKey="ft03Decision"`），事件訂閱由 `OnEnable` / `OnDisable` 生命週期管理，與 FT-10 restore 流程無關 | — |

### 2.5 跨系統事件契約

| 事件名稱 | 方向 | Payload | 發布時機 / 訂閱目的 |
| --- | --- | --- | --- |
| `OnMinuteTickEvent` | 訂閱（來自 F-02） | `long NowUTC`（typed struct）| 驅動 `AutoPickupTick`，每分鐘掃描 Idle 冒險者（FSD-Codex-Reoprts-260427 T1-FT03 修正：原寫 `float nowUTC` 與既有 typed struct 不符） |
...
| jitter 製造「NPC 有時做次優決策」 | 邊緣案例（分數接近門檻）下接受/拒絕結果不完全可預測 | `_rng.NextFloat(-WILLINGNESS_JITTER, +WILLINGNESS_JITTER)` 在 Step 4 加入；`NpcDecisionService` 持有 `System.Random _rng`（建構子可注入測試 seed，FSD-Codex-Reoprts-260427 T1-FT03 補：AC-ND3-01 / AC-ND3-03 用固定 seed 確保可重現） |
...
| --- | --- | --- | --- | --- |
| `NpcDecisionTypes.cs` | `Assets/Scripts/Gameplay/Decision/NpcDecisionTypes.cs` | 定義 `DecisionResult`、`RejectionReason` enum | 無（純資料型別） | < 50 行 |
...
| `NpcDecisionEvents.cs` | `Assets/Scripts/Gameplay/Decision/Events/NpcDecisionEvents.cs` | 集中宣告 typed event struct：`OnAutoPickupEvent { int adventurerInstanceID; int missionID; }` | — | < 50 行 |
...
| 事件名稱 | 方向 | Payload | 發布時機 / 訂閱目的 |
| --- | --- | --- | --- |
| `OnMinuteTickEvent` | 訂閱（F-02 發出） | `long NowUTC` | 驅動 `AutoPickupTick`，每「分鐘」（= 60 秒，F-02 alias）掃描一次 |
...
```
F-02.OnMinuteTickEvent(evt.NowUTC : long)
...
| --- | --- | --- | --- | --- |
| `SystemConstants.csv` | `DEATH_AVERSION`、`ACCEPTANCE_THRESHOLD`、`WILLINGNESS_JITTER`、`AUTO_PICKUP_IDLE_MINUTES`、`AUTO_PICKUP_INTERVAL_MINUTES` | `【F-01-DS】system-constants.md` | effectiveScore 公式常數；自主接單觸發與間隔閾值 | `NpcDecisionService.Awake()`（透過 `IDataManager.GetFloat(key)` 載入並快取） |
...
| §5.2 存檔讀取後 lastAutoPickupTimestamp = 0 | **觸發節奏由 `idleSinceTimestamp` 決定**（FSD-Codex-Reoprts-260427 T1-FT03 修正）：(a) 若存檔的 `idleSinceTimestamp` 已超過 `_autoPickupIdleSeconds`，重啟後第一個 `OnMinuteTickEvent` 即觸發 `TryAutoPickup`；(b) 若 `idleSinceTimestamp` 未滿，等待 `nowUTC - idleSinceTimestamp >= _autoPickupIdleSeconds` 才觸發。`lastAutoPickupTimestamp = 0` 僅讓 `sinceLastPickup` 守衛通過，**不延遲首次觸發** | `NpcDecisionService` | EditMode test：設 `idleSinceTimestamp = nowUTC - 700`、`_autoPickupIdleSeconds = 600` → 第一 tick 即觸發；改 `idleSinceTimestamp = nowUTC - 100` → 不觸發 |
