...
- 三軌補池觸發（啟動填池 / F-02 `OnDailyReset` / C-06 `OnDangerLevelChanged`）
...
| F-02 Time System | 訂閱 `OnDailyReset` 驅動每日補池 | `OnDailyReset` 事件 |
| C-06 World Danger System | 訂閱 `OnDangerLevelChanged` 驅動危險度升階補池；查詢池權重 | `OnDangerLevelChanged` 事件、`IC06WorldDanger.GetPoolWeights()` |
...
| --- | --- | --- | --- |
| `OnDailyReset` | 訂閱（F-02） | — | `OnEnable` 訂閱；觸發每日重置補池 |
| `OnDangerLevelChanged` | 訂閱（C-06） | `int newDangerLevel` | `OnEnable` 訂閱；觸發危險度升階補池 |
| `OnCommissionPosted` | 發布 | `(int missionID, CommissionSource source)` | `PostRegularMission` 或 `InjectStaticMission` 成功後 |
...
| 每天有新委託出現 | 委託板每日刷新，缺額自動補上 | 訂閱 `OnDailyReset`，計算 deficit 並補池 |
| 局勢緊張時高難度委託增多 | 危險度升階後委託難度分布改變 | 訂閱 `OnDangerLevelChanged`，依新權重重新補池 |
...
| 事件名稱 | 方向 | Payload | 發布時機 / 訂閱目的 |
| --- | --- | --- | --- |
| `OnDailyReset` | 訂閱（F-02） | — | `OnEnable` 訂閱；觸發 `RefillPool(deficit)` |
| `OnDangerLevelChanged` | 訂閱（C-06） | `int newDangerLevel` | `OnEnable` 訂閱；觸發 `RefillPool(deficit)` |
| `OnCommissionPosted` | 發布 | `(int missionID, CommissionSource source)` | `PostRegularMission` / `InjectStaticMission` 成功後 |
...
// 觸發點 2：F-02.OnDailyReset callback
// 觸發點 3：C-06.OnDangerLevelChanged callback
...
**建議 B-01（categoryID magic number）**

GDD §3.9.2 直接使用數值 `3`（Static）與 `0`（Regular）驗證 categoryID。FT-02-B 程式碼應以命名常數包裝（例如：從 C-01 介面取得或定義在 `CommissionBoardEnums.cs`），避免邏輯分支中出現裸數字。不阻礙實作，建議 Codex 實作時一併處理。

**建議 B-02（兩池合集去重保證）**

`IsCommissionOnBoard` 已跨兩池判斷，正常流程不應產生重複。`GetAvailableCommissions()` 實作時建議以 `Enumerable.Concat` 合集，不加額外去重（依靠 `IsCommissionOnBoard` 上游保護）；若未來需 UI 排序，可在 UI 層處理而非在此 API 中。
