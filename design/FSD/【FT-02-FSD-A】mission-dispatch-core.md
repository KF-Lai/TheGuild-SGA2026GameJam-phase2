...
| AC | 可驗證條件 |
| --- | --- |
| AC-MD2-01 | EditMode test：C 階戰士 vs B 難度討伐，`CalculateRates` 回傳 `(0.55, 0.18)` |
...
| `MissionTimer`（既有，建議遷移） | 現位於 `Assets/Scripts/Core/Time/MissionTimer.cs`；建議遷移至 `Assets/Scripts/Gameplay/Mission/MissionTimer.cs` | （待確認職責；見 §8.3 偏差登記） | — | — |
...
**ISaveable 序列化 payload（FT-02-A 部分）**
...
序列化欄位：
  ...
  _nextActiveMissionID    : int
```

> `_regularMissionPool`、`_staticMissionPool` 序列化由 FT-02-B `CommissionBoardService` 負責，同屬 `"ft02Dispatch"` owner key，Serialize/Restore 分工在同一 owner 下協調（FT-02-B §5.3 說明）。
...
- GDD §4.2 範例（C 階戰士 vs B 難度討伐 → 0.63 / 0.18）：FSD 不嵌入測試值，由 AC-MD2-01 EditMode test 覆蓋。

### 8.3 未能實現的規則與修改建議

**偏差 D-01（MissionTimer.cs 位置偏差）**

- 現況：`Assets/Scripts/Core/Time/MissionTimer.cs` 位於 Core/Time 目錄，但其職責（任務計時）屬 FT-02 範疇。
- 建議：遷移至 `Assets/Scripts/Gameplay/Mission/MissionTimer.cs`；若 MissionTimer 職責已被 `MissionDispatchService.TickCompletionCheck` 完整替代，則移除 `MissionTimer.cs`（需閱讀其內容確認後再決策）。
- 優先順序：實作 `MissionDispatchService` 時確認，遷移或移除前先以 Glob 確認 MissionTimer 依賴。
