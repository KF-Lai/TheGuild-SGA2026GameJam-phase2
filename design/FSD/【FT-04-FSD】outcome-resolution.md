...
- 自行套用 condition 特質（讀取冒險者 `traitIDs` 中 `effectType == "condition"` 的特質，套用救活、聲望 bonus、gold bonus 填值；FSD-Codex-Reoprts-260427 CT-06 裁決：FT-04 為 owner，C-05 不暴露 `ApplyConditionTraits`）
...
| EditMode test：`finalSuccessRate=0.5` + 固定 seed 1000 次擲骰，成功比率 50% ±5%（FSD-Codex-Reoprts-260427 T1-FT04 補 AC-OR-10 機率分佈） | NUnit + 注入 seed |
| EditMode test：`successRoll` 與 `deathRoll` 兩次擲骰獨立性（同 seed 下兩次呼叫 RNG 產生不同 roll，且改變 successRoll 不影響 deathRoll；T1-FT04 補 AC-OR-12） | NUnit |
| EditMode test：condition trait `on_success_gold_bonus` 觸發後 `outcome.conditionGoldBonus == trait.effectValue`；FT-04 不呼叫 AddGold（T1-FT04 補 AC-OR-17） | NUnit + mock IResourceService 確認 AddGold 未被呼叫 |
| EditMode test：F-03 `AddReputation` 呼叫的 delta 經 F-03 內部 clamp 至 [REPUTATION_MIN, REPUTATION_MAX]（T1-FT04 補 AC-OR-23） | NUnit + mock IResourceService 觀察 clamp 後值 |
| EditMode test：離線批次 — mock `OnMissionCompleted` 對 5 個 activeMissionID 連發，5 個 `OnMissionResolved` 依序發布、`activeMissions` 全清空（T1-FT04 補 AC-OR-27/28） | NUnit |
...
| §3.4 condition 特質套用 | FT-04 內部 `ApplyConditionTraits(outcome, traitIDs)` 流程；C-05 僅提供 `GetTrait(id)` / `GetTraitsByType("condition")` 查詢 |
...
| 系統 | 呼叫／訂閱介面 | 用途 |
| --- | --- | --- |
| **F-01 DataManager** | `RegisterTable<ReputationDeltaData>("ReputationDeltaTable")` 於 `[RuntimeInitializeOnLoadMethod(BeforeSceneLoad)]` 註冊；`IDataManager.GetAll<ReputationDeltaData>()` 取得清單後 Calculator 自建 `Dictionary<string, ReputationDeltaEntry>`；`IDataManager.GetFloat("DEATH_RATE_ON_SUCCESS_MULTIPLIER")` 取得死亡率折扣係數 | 載入聲望表與系統常數 |
...
| **C-05 Trait System** | `ITraitService.GetTrait(int traitID)` 取得單筆特質定義；`ITraitService.GetTraitsByType("condition")`（測試用，非結算路徑必需） | 取 condition 特質定義；**FT-04 自行實作 condition 套用**（FSD-Codex-Reoprts-260427 CT-06 裁決） |
...
| **FT-09 Faction Story** | 訂閱 `OnMissionResolved` | `missionFactionID`, `isSuccess`, `missionDifficulty`, `missionID`, `isDead`, `missionTypeID`（FSD-Codex-Reoprts-260427 T1-FT04 補：對齊 FT-09 GDD 完整消費欄位） |
...
| condition 特質在最後改變命運 | 「亡命徒特質觸發，死亡 → 被救活」訊息鏈 | `OutcomeResolutionService.ApplyConditionTraits(outcome, traitIDs)`（FT-04 內部 method）在擲骰後、`MapFinalStatus` 前修改 `isDead` / `isWounded` |
...
| `OutcomeEvents.cs` | `Assets/Scripts/Gameplay/Outcome/Events/OutcomeEvents.cs` | 集中宣告 typed event struct：`OnMissionResolvedEvent { Outcome outcome; }` | — | `< 50 行` |
...
OutcomeResolutionService
  ...
  ├─ 自行 ApplyConditionTraits（內部 method，逐筆讀 ITraitService.GetTrait(id) 後套用）
...
OutcomeReputationCalculator
  └─ 使用 IDataManager.GetAll<ReputationDeltaData>() → 內部建 Dictionary<string, ReputationDeltaEntry>
```
...
F-01 `GetAll<ReputationDeltaData>()` 回傳 `IReadOnlyList<ReputationDeltaData>`；`OutcomeReputationCalculator` 於 `Awake` 內建立 `Dictionary<string, ReputationDeltaEntry>`，PK 為 `difficulty` 字串（F~SSS 共 9 列）。
...
EventBus → OutcomeResolutionService.OnMissionCompleted(activeMissionID)
    ...
    │           //     triggered.Add(traitID)
    │           //     switch trait.effectTarget:
    │           //       "on_death_survive":      if outcome.isDead → outcome.isDead=false; outcome.isWounded=true
    │           //       "on_success_gold_bonus": if outcome.isSuccess → outcome.conditionGoldBonus += trait.effectValue
    │           //       "on_success_reputation": if outcome.isSuccess → outcome.reputationDelta += trait.effectValue
    │           //       "on_fail_reputation":    if !outcome.isSuccess → outcome.reputationDelta += trait.effectValue
    │           //       "on_death_reputation":   if outcome.isDead → outcome.reputationDelta += trait.effectValue
    │           //       未知 effectTarget → LogWarning, skip
    │           //   outcome.triggeredConditionTraits = triggered
...
OutcomeResolutionService.Awake()
    → _deathRateMultiplier = IDataManager.GetFloat("DEATH_RATE_ON_SUCCESS_MULTIPLIER")
      if DataManager 內部 LogError（key 不存在，回傳 0f）→ _deathRateMultiplier = 0.5f (fallback)
...
| **§5.4** 冒險者無任何 condition 特質 | `ApplyConditionTraits`（FT-04 內部）以空 traitIDs 呼叫，迴圈不執行；`conditionGoldBonus == 0`、`triggeredConditionTraits.Length == 0`、`reputationDelta == baseDelta` | `OutcomeResolutionService` | EditMode test：mock 空 traitIDs → 驗證三欄位 |
...
| **§5.4** condition 特質 `effectTarget` 為未知值 | FT-04 `ApplyConditionTraits` 內部 switch default 分支 `LogWarning` 並跳過，繼續處理其他特質 | `OutcomeResolutionService` | EditMode test：注入 unknown effectTarget trait → 驗證 LogWarning + 不影響其他特質結果 |
...
| 2026-04-27 | `DEATH_RATE_ON_SUCCESS_MULTIPLIER` fallback 值衝突：GDD AC-OR-02 寫「缺失時預設 `1.0`」；FT-04 FSD §5.4 / §7 採 `0.5`（任務成功時死亡率減半，符合「成功路徑死亡率打折鼓勵冒險」設計幻想）。FSD-Codex-Reoprts-260427 CT-08 使用者裁決：**採 FSD `0.5` 為準**；GDD AC-OR-02 應更新為 `0.5` 或移除預設值描述（待 GDD 同步回註） | FT-04 GDD AC-OR-02 / FT-04 FSD §5.4 §7 / `【F-01-DS】system-constants.md` | 採 FSD `0.5`；已於 `SystemConstants.csv` 寫入 `DEATH_RATE_ON_SUCCESS_MULTIPLIER=0.5`；§8.4 待新增 GDD 回註 |
