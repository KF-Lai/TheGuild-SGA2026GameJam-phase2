...
| 2026-05-02 | IPanel registry / Open(args) dispatch 實作落差 | P-02-FSD-A vs P-02-FSD-B | 採用 chain continue 相容修補方向：PanelManager 增加 `RegisterPanel(IPanel)` API + `OpenPanel` 成功後 dispatch `IPanel.Open(args)` + `ClosePanel` onClosed callback dispatch `IPanel.Close()`，避免 FSD-B 面板只能註冊 root 而收不到 args；同步記錄 §3/§5 的 API 落差。 |
...
| 2026-05-02 | Claude Code 主體（Opus 4.7 + xhigh）—— FSD-B B1 面板實作寫入（commit pending）| 通過 | 通過 | 通過 | chain continue 相容修補：新增 PanelManager `IPanel.Open(args)` dispatch hook；PanelManager.cs 增加 `_panels: Dictionary<PanelID, IPanel>` registry + `RegisterPanel(IPanel)` API + OpenPanel 成功後 dispatch + ClosePanel onClosed dispatch。Panel interface 放在 `TheGuild.UI.Core` namespace，供 `TheGuild.UI.Panels` asmdef 消費；FSD §5.4.2 原規格未明示 PanelManager 持有 IPanel registry，本輪以實作落差修補方式補齊。 |
