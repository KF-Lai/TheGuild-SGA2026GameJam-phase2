...
| F-02 Time System | `ITimeSystem.NowUTC`、`OnDailyReset`（訂閱）、`OnSecondTick`（訂閱） | 自動刷新計時、每日免費刷新次數重置 |
...
| C-02 Adventurer Management | `IAdventurerRoster.CreateFromTemplate(templateID)`、`IAdventurerRoster.AddAdventurer(instance)`、`IAdventurerRoster.IsRosterFull(rosterCap)`、`IAdventurerRoster.GetRoster()`、`IAdventurerRoster.CreateRandomInstance(rank, professionID, raceID, traitIDs)`、`IAdventurerRoster.BuildTraitList` | 候選冒險者建立、加入名冊、名冊容量檢查、名冊 templateID 快照 |
...
| `OnDailyReset` | F-02 → EventBus（FT-01 訂閱） | — | FT-01 訂閱以重置 `_freeRefreshRemaining` |
...
| `RecruitmentPoolGenerator.cs` | `Assets/Scripts/Gameplay/Recruitment/RecruitmentPoolGenerator.cs` | 依照職責（新手/老手）生成候選者列表，執行具名模板篩選與加權隨機 | `IAdventurerRoster`、`IProfessionService`、`IRaceService`、`ITraitService`、`IGuildCoreService`、`DataManager` | 200~280 行 |
...
UI.ManualRefresh()
  → RecruitmentService.ManualRefresh() : bool
      ...
      │     IResourceService.AddGold(-REFRESH_COST)
      ├─ ExecuteRefresh()
...
UI.RecruitVeteran(candidateID)
  → RecruitmentService.RecruitVeteran(candidateID) : bool
      ...
      ├─ IResourceService.AddGold(-candidate.Cost)    // 扣款
      ├─ result = IAdventurerRoster.AddAdventurer(candidate.AdventurerInstance)
      ├─ if (result == true):
      ...
      │     return true
      └─ if (result == false):
            ├─ IResourceService.AddGold(+candidate.Cost)    // 回滾扣款
